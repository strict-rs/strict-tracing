    ...
    W: Write + Send + 'static,
{
            ...
            while let Ok(command) = receiver.recv() {
                match command {
                    ...
                    WriterCommand::Flush(flush_complete) => {
                        ...
                        let _send_result: Result<(), mpsc::SendError<io::Result<()>>> =
                            flush_complete.send(flush_result);
                    }
                }
            }
        });
    ...
    fn write_line(&self, line: String) {
        let _send_result: Result<(), mpsc::SendError<WriterCommand>> =
            self.commands.send(WriterCommand::WriteLine(line));
    }
    ...
    W: Write + Send + 'static,
{
    ...
    pub const fn with_module_path(mut self, enabled: bool) -> Self {
        self.config.module_path = if enabled {
            ModulePath::Include
        } else {
            ModulePath::Omit
        };
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    ) -> Result<(Self, FlushGuard<BufWriter<File>>), FlameError> {
        let output_path = path.as_ref();
        let file = File::create(output_path)
            .map_err(|source| FlameError::create_file(output_path.to_path_buf(), source))?;
