...
`tracing-test` is a tiny (~64 LOC) **internal, `publish = false`** test-support crate — distinct from the unrelated third-party `tracing-test` on crates.io. Per its manifest's "BIG SCARY NOTE" it must never be published; it exists so async-future test helpers can be shared without bloating `tracing-mock`. Workspace-wide build/test/feature/commit conventions live in the root `AGENTS.md`.
