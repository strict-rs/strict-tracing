...
- `rolling.rs` / `rolling/builder.rs` — `RollingFileAppender` (blocking `Write` + `MakeWriter`) rotates per `Rotation` (`MINUTELY`/`HOURLY`/`DAILY`/`WEEKLY`/`NEVER`, all UTC). The free fns `minutely`/`hourly`/`daily`/`weekly`/`never`, `RollingFileAppender::new()`, and `Builder::build()` are fallible and return `Result<_, InitError>`. `Builder` adds `filename_prefix`/`filename_suffix`/`max_log_files`/`latest_symlink`. Rollover timing is a lock-free `AtomicUsize` (`next_date` as a unix timestamp; `0` means never-rotate) advanced via `compare_exchange`. Date formats are parsed with `time::format_description::parse_borrowed::<1>`, so the manifest must keep `time`'s `formatting` and `parsing` features.
...
- `RollingFileAppender::new()`, the free constructors, and `Builder::build()` are fallible and return `InitError` on initialization failure.
