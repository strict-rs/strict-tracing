...
//! Logs will be written to this file for the duration of the period and will automatically roll
//! over to the newly created log file once the time period has elapsed.
//!
//! The log file is created at the specified directory and file name prefix which *may* be appended
//! with the date and time.
//!
//! The following helpers are available for creating a rolling file appender.
//!
//! - [`Rotation::minutely()`][minutely]: A new log file in the format of
//!   `some_directory/log_file_name_prefix.yyyy-MM-dd-HH-mm` will be created minutely (once per
//!   minute)
//! - [`Rotation::hourly()`][hourly]: A new log file in the format of
//!   `some_directory/log_file_name_prefix.yyyy-MM-dd-HH` will be created hourly
//! - [`Rotation::daily()`][daily]: A new log file in the format of
//!   `some_directory/log_file_name_prefix.yyyy-MM-dd` will be created daily
//! - [`Rotation::never()`][never()]: This will result in log file located at
//!   `some_directory/log_file_name`
//!
//!
//! # Examples
//!
...
impl RollingFileAppender {
  ...
  pub fn new(rotation: Rotation, directory: impl AsRef<Path>, filename_prefix: impl AsRef<Path>) -> Result<Self, InitError> {
    let filename_prefix_text = filename_prefix
      .as_ref()
      .to_str()
      .ok_or_else(|| InitError::ctx("filename prefix must be a valid UTF-8 string")(io::ErrorKind::InvalidInput.into()))?;
    Self::builder()
      .rotation(rotation)
...
/// Returns [`InitError`] when the filename prefix is not valid UTF-8 or the
/// initial log file cannot be created.
pub fn minutely(directory: impl AsRef<Path>, file_name_prefix: impl AsRef<Path>) -> Result<RollingFileAppender, InitError> {
...
/// Returns [`InitError`] when the filename prefix is not valid UTF-8 or the
/// initial log file cannot be created.
pub fn hourly(directory: impl AsRef<Path>, file_name_prefix: impl AsRef<Path>) -> Result<RollingFileAppender, InitError> {
...
/// Returns [`InitError`] when the filename prefix is not valid UTF-8 or the
/// initial log file cannot be created.
pub fn daily(directory: impl AsRef<Path>, file_name_prefix: impl AsRef<Path>) -> Result<RollingFileAppender, InitError> {
...
/// Returns [`InitError`] when the filename prefix is not valid UTF-8 or the
/// initial log file cannot be created.
pub fn weekly(directory: impl AsRef<Path>, file_name_prefix: impl AsRef<Path>) -> Result<RollingFileAppender, InitError> {
...
pub fn never(directory: impl AsRef<Path>, file_name: impl AsRef<Path>) -> Result<RollingFileAppender, InitError> {
...
impl Inner {
  ...
  fn new(
    ...
    let inner = Self {
      ...
      date_format,
      next_date: AtomicI64::new(next_date.map_or(NEVER_ROLLOVER_TIMESTAMP, OffsetDateTime::unix_timestamp)),
      rotation,
      max_files,
    };
  ...
  fn refresh_writer(&self, now: OffsetDateTime, file: &mut File) -> io::Result<()> {
    ...
    let new_file =
      create_writer(&self.log_directory, &filename, self.log_latest_symlink_name.as_deref()).map_err(|_error| other_error())?;
    file.flush()?;
    *file = new_file;
    Ok(())
  }
  ...
  fn restore_date_after_failed_rollover(&self, attempted: i64, previous: i64) {
    let _restore_result = self
      .next_date
      .compare_exchange(attempted, previous, Ordering::AcqRel, Ordering::Acquire);
  }
...
fn create_writer(directory: &Path, filename: &str, latest_symlink_name: Option<&str>) -> Result<File, InitError> {
  ...
  let new_file = match options.open(&path) {
    ...
    Err(error) if error.kind() == io::ErrorKind::NotFound => {
      ...
      options.open(&path).map_err(InitError::ctx("failed to create log file"))?
    }
  ...
  if let Some(symlink_name) = latest_symlink_name {
    ...
    {
      return Err(InitError::ctx("failed to remove previous latest log symlink")(error));
    }
    symlink::symlink_file(path, symlink_path).map_err(InitError::ctx("failed to create symlink to latest log file"))?;
  }
...
fn parse_date_from_filename(
  ...
  PrimitiveDateTime::parse(datetime, date_format)
    .or_else(|_| Date::parse(datetime, date_format).map(|parsed_date| parsed_date.with_time(Time::MIDNIGHT)))
    .ok()
...
mod test {
  ...
  fn test_appender(rotation: Rotation, file_prefix: &str) -> Result<(), TestFailure> {
    ...
    ensure_ok(appender.write_all(expected_value.as_bytes()), "write to appender")?;
  ...
  fn clocked_now(clock: &Arc<Mutex<OffsetDateTime>>) -> Box<dyn Fn() -> OffsetDateTime + Send + Sync> {
