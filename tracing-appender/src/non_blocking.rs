...
impl NonBlocking {
    ...
    pub fn try_new<T: Write + Send + 'static>(
        writer: T,
    ...
    fn create<T: Write + Send + 'static>(
        ...
        let worker_guard = WorkerGuard::new(
            Some(worker.worker_thread(thread_name)?),
            sender.clone(),
            shutdown_sender,
        );
    ...
    fn create_failed(
        buffered_lines_limit: usize,
        is_lossy: bool,
        worker_error_kind: io::ErrorKind,
    ) -> (Self, WorkerGuard) {
        let (sender, receiver) = bounded(buffered_lines_limit);
        let (shutdown_sender, shutdown_receiver) = bounded(0);
        drop(receiver);
...
impl NonBlockingBuilder {
    ...
    pub fn try_finish<T: Write + Send + 'static>(
        self,
        writer: T,
    ) -> Result<(NonBlocking, WorkerGuard), WorkerSpawnError> {
        NonBlocking::create(
            writer,
...
impl Drop for WorkerGuard {
    fn drop(&mut self) {
        match self
            .sender
            .send_timeout(Msg::Shutdown, SHUTDOWN_SIGNAL_TIMEOUT)
        {
            Ok(()) => {
...
mod test {
    ...
    fn join_test_thread(
        handle: JoinHandle<Result<(), TestFailure>>,
        context: &'static str,
    ) -> Result<(), TestFailure> {
        match handle.join() {
            Ok(result) => result,
            Err(_panic) => Err(TestFailure::Condition { context }),
        }
    }
    ...
    fn backpressure_exerted() -> Result<(), TestFailure> {
        ...
        ensure_eq(
            &error_count.dropped_lines(),
            &0,
            "initial write does not drop lines",
        )?;

        let handle = thread::spawn(move || {
            ensure_ok(non_blocking.write_all(b", World"), "write blocked line")
        });
        ...
        ensure_eq(
            &error_count.dropped_lines(),
            &0,
            "blocked write does not drop lines",
        )?;
    ...
    fn logs_dropped_if_lossy() -> Result<(), TestFailure> {
        ...
        ensure_eq(
            &error_count.dropped_lines(),
            &0,
            "first lossy write does not drop lines",
        )?;
        ...
        ensure_eq(
            &error_count.dropped_lines(),
            &0,
            "second lossy write does not drop lines",
        )?;
        ...
        ensure_eq(
            &error_count.dropped_lines(),
            &0,
            "buffered lossy write does not drop lines",
        )?;
        ...
        ensure_eq(
            &error_count.dropped_lines(),
            &1,
            "full lossy queue drops one line",
        )?;
        ...
        ensure_eq(
            &error_count.dropped_lines(),
            &1,
            "lossy dropped-line count remains one",
        )
    }
    ...
    fn multi_threaded_writes() -> Result<(), TestFailure> {
        ...
        let error_count = non_blocking.error_counter();
        let mut join_handles: Vec<JoinHandle<Result<(), TestFailure>>> =
        ...
        ensure_eq(
            &hello_count,
            &THREAD_COUNT,
            "all writer threads emit one line",
        )?;
        ensure_eq(
            &error_count.dropped_lines(),
            &0,
            "multi-threaded writes do not drop lines",
        )
    }
}
