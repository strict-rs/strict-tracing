...
- `expand.rs` — codegen. `gen_function` → `gen_block` emit the span-creating wrapper; `AsyncInfo::from_fn` / `gen_async` rewrite async-trait-style bodies (`async-trait <= 0.1.43`'s inner `async fn` + `Box::pin`, and `>= 0.1.44`'s `Box::pin(async move {...})`) so the *inner future* is instrumented rather than the allocating wrapper.
