...
mod nested_target_tests {
  ...
  fn default_targets() -> Result<(), TestFailure> {
    let (subscriber, handle) = subscriber::mock()
      ...
      .exit(expect::span().named("default_target").with_target(ROOT_MODULE_PATH))
      .new_span(expect::span().named("nested_default_target").with_target(MODULE_PATH))
      .enter(expect::span().named("nested_default_target").with_target(MODULE_PATH))
      .exit(expect::span().named("nested_default_target").with_target(MODULE_PATH))
      .only()
      .run_with_handle();
  ...
  fn custom_targets() -> Result<(), TestFailure> {
    let (subscriber, handle) = subscriber::mock()
      ...
      .exit(expect::span().named("custom_target").with_target("my_target"))
      .new_span(expect::span().named("nested_custom_target").with_target("my_other_target"))
      .enter(expect::span().named("nested_custom_target").with_target("my_other_target"))
      .exit(expect::span().named("nested_custom_target").with_target("my_other_target"))
      .only()
      .run_with_handle();
