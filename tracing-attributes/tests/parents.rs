...
fn default_parent_test() -> Result<(), TestFailure> {
  ...
  let (subscriber, handle) = subscriber::mock()
    .new_span(contextual_parent.clone().with_ancestry(expect::is_contextual_root()))
    .new_span(child.clone().with_ancestry(expect::is_contextual_root()))
    .enter(child.clone())
    .exit(child.clone())
    .enter(contextual_parent.clone())
    .new_span(child.clone().with_ancestry(expect::has_contextual_parent("contextual_parent")))
    .enter(child.clone())
    .exit(child)
...
fn explicit_parent_test() -> Result<(), TestFailure> {
  ...
  let (subscriber, handle) = subscriber::mock()
    ...
    .enter(contextual_parent.clone())
    .new_span(child.clone().with_ancestry(expect::has_explicit_parent("explicit_parent")))
    .enter(child.clone())
    .exit(child)
    .exit(contextual_parent)
    .only()
