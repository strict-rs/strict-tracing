...
mod tests {
    ...
    fn read_from_journal(
        ...
        test_name: &str,
    ) -> Result<Vec<HashMap<String, Field>>, TestFailure> {
    ...
    fn retry_read_one_line_from_journal(
        testname: &str,
    ) -> Result<HashMap<String, Field>, TestFailure> {
    ...
    fn retry_read_one_line_from_user_journal(
        testname: &str,
    ) -> Result<HashMap<String, Field>, TestFailure> {
    ...
    fn field<'a>(
        message: &'a HashMap<String, Field>,
        name: &'static str,
    ) -> Result<&'a Field, TestFailure> {
    ...
    fn ensure_binary(
        ...
        expected: &[u8],
            ...
            field(message, name)?.bytes_eq(expected),
            "journal binary field matches",
        )
    }
    ...
    fn ensure_text_present(
        message: &HashMap<String, Field>,
        name: &'static str,
    ) -> Result<(), TestFailure> {
        ensure(
            field(message, name)?.as_text().is_some(),
            "journal text field is present",
        )
    }
    ...
    fn multiple_spans_metadata() -> Result<(), TestFailure> {
        with_journald(|| {
            ...
            let span_code_line = ensure_some(
                field(&message, "SPAN_CODE_LINE")?.as_array(),
                "span code line is array",
            )?;
            ensure(
                span_code_line.len() == 2,
                "span code line contains both spans",
            )
        })
    }
