...
- `socket.rs` `send_one_fd_to` passes a single fd as an `SCM_RIGHTS` control message through `rustix` (`SocketAddrUnix`, `SendAncillaryBuffer`, `sendmmsg`) and rejects paths at Linux `sun_path` capacity before sending. `memfd.rs` uses `rustix::fs::memfd_create` plus `fcntl_add_seals` and accepts `AsFd`; together they implement the oversized-payload path when `send_to` returns `rustix::io::Errno::MSGSIZE`.
...
- `Layer` has a manual `Debug` impl that includes `socket_path` only on Unix and otherwise reports configuration fields. Keep `JournalNamespace` behavior aligned with the socket path in both normal sends and memfd fallback sends.
