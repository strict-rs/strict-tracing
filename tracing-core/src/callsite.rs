...
//! * Caching the subscriber's [`Interest`][^1] in that span or event, to avoid re-evaluating
//!   filters.
//!
...
use crate::metadata::LevelFilter;
...
impl DefaultCallsite {
  ...
  pub fn register(&'static self) -> Interest {
    ...
    match self
      .registration
      .compare_exchange(Self::UNREGISTERED, Self::REGISTERING, Ordering::AcqRel, Ordering::Acquire)
    {
      Ok(_) => {
      ...
      Err(_state) => {
        self.registration.store(Self::UNREGISTERED, Ordering::Release);
        return Interest::sometimes();
      }
    }
...
impl Callsites {
  ...
  fn push(&self, callsite: &'static dyn Callsite) {
    ...
    if registered.iter().any(|existing| ptr::addr_eq(*existing, callsite)) {
      return;
    }
...
fn rebuild_callsite_interest(callsite: &'static dyn Callsite, dispatchers: &dispatchers::Rebuilder<'_>) {
  ...
  dispatchers.for_each(|dispatch| {
    ...
    combined_interest = combined_interest
      .take()
      .map_or(Some(this_interest), |that_interest| Some(that_interest.and(this_interest)));
  });
...
mod dispatchers {
  ...
  use super::sync::RwLockReadGuard;
  use super::sync::RwLockWriteGuard;
  ...
  static LOCKED_DISPATCHERS: RwLock<Vec<dispatcher::Registrar>> = parking_lot::const_rwlock(Vec::new());
  ...
  impl Dispatchers {
    ...
    pub(super) fn register_dispatch(&self, dispatch: &dispatcher::Dispatch) -> Rebuilder<'_> {
      ...
      self.has_just_one.store(dispatchers.len() <= 1, Ordering::SeqCst);
...
mod dispatchers {
  ...
  impl Dispatchers {
    ...
    pub(super) fn register_dispatch<'dispatch>(&self, dispatch: &'dispatch dispatcher::Dispatch) -> Rebuilder<'dispatch> {
