...
/// style="white-space:normal;font:inherit;">     <strong>Warning</strong>: In general, libraries
/// should <em>not</em> call     <code>set_global_default()</code>! Doing so will cause conflicts
/// when     executables that depend on the library try to set the default later.
/// </pre></div>
///
/// [span]: super::span
/// [`Subscriber`]: super::subscriber::Subscriber
...
pub fn set_global_default(dispatcher: Dispatch) -> Result<(), SetGlobalDefaultError> {
    ...
    .is_ok()
  {
    ...
    if GLOBAL_DISPATCH.set(dispatcher).is_err() {
      ...
      return Err(SetGlobalDefaultError {
        _no_construct: ()
      });
    }
    #[cfg(not(feature = "std"))]
...
impl fmt::Debug for SetGlobalDefaultError {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    f.debug_tuple("SetGlobalDefaultError").field(&Self::MESSAGE).finish()
  }
}
...
impl Dispatch {
  ...
  #[inline]
  pub fn register_callsite(&self, metadata: &'static Metadata<'static>) -> SubscriberResult<subscriber::Interest> {
...
impl WeakDispatch {
  ...
  pub fn upgrade(&self) -> Option<Dispatch> {
    self.subscriber.upgrade().map(|subscriber| Dispatch {
      subscriber,
    })
  }
}
...
mod test {
  ...
  fn spans_dont_infinite_loop() -> Result<(), TestFailure> {
    ...
    fn mk_span() {
      let _span = get_default(|current| current.new_span(&span::Attributes::new(&TEST_META, &TEST_META.fields().value_set(&[]))));
    }
