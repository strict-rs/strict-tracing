...
/// - Registering new spans as they are created, and providing them with span IDs. Implicitly, this
///   means the subscriber may determine the strategy for determining span equality.
...
///
/// - [`register_callsite`] is called once for each callsite from which a span event may originate,
///   and returns an [`Interest`] value describing whether or not the subscriber wishes to see
///   events or spans from that callsite. By default, it calls [`enabled`], and returns
///   `Interest::always()` if `enabled` returns true, or `Interest::never()` if enabled returns
///   false. However, if the subscriber's interest can change dynamically at runtime, it may want to
///   override this function to return `Interest::sometimes()`. Additionally, subscribers which wish
///   to perform a behaviour once for each callsite, such as allocating storage for data related to
///   that callsite, can perform it in `register_callsite`.
///
///   See also the [documentation on the callsite registry][cs-reg] for details
///   on [`register_callsite`].
///
/// - [`event_enabled`] is called once before every call to the [`event`] method. This can be used
///   to implement filtering on events once their field values are known, but before any processing
///   is done in the `event` method.
/// - [`clone_span`] is called every time a span ID is cloned, and [`try_close`] is called when a
///   span ID is dropped. By default, these functions do nothing. However, they can be used to
///   implement reference counting for spans, allowing subscribers to free storage for span data and
///   to determine when a span has _closed_ permanently (rather than being exited). Subscribers
///   which store per-span data or which need to track span closures should override these functions
///   together.
///
/// [ID]: super::span::Id
...
pub trait Subscriber: AsAny {
  ...
  /// If `self` is the same type as the provided `TypeId`, or contains a
  /// component with that `TypeId`, returns that value as `dyn Any`.
  ///
  /// This method is the object-safe implementation hook used by
  /// [`downcast_ref`]. Subscriber implementations that compose multiple
  /// subscribers may override this to expose safe references to contained
  /// components.
  ///
...
impl Interest {
  ...
  pub(crate) fn and(self, rhs: Self) -> Self {
    if self.0 == rhs.0 { self } else { Self::sometimes() }
  }
}
