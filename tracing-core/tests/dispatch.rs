...
mod tests {
  ...
  fn nested_set_default() -> Result<(), TestFailure> {
    ...
    get_default(|current| ensure(current.is::<TestSubscriberA>(), "set_default for outer subscriber failed"))?;

    let inner_guard = set_default(&Dispatch::new(TestSubscriberB));
    get_default(|current| ensure(current.is::<TestSubscriberB>(), "set_default inner subscriber failed"))?;

    drop(inner_guard);
    get_default(|current| ensure(current.is::<TestSubscriberA>(), "set_default outer subscriber failed"))
  }
}
