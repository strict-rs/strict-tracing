...
use tracing::{
    ...
    subscriber::{SetGlobalDefaultError, SubscriberResult, set_global_default},
};
...
impl Subscriber for SloggishSubscriber {
    ...
    fn enter(&self, span_id: Id) -> SubscriberResult {
        ...
        let (maybe_parent, fields_to_print) = {
            ...
            let span_parts = known_spans.get(&span_id).map_or_else(
                || (None, Vec::new()),
                |span_data| (span_data.parent, span_data.fields.clone()),
            );
            drop(known_spans);
            span_parts
        };
        ...
        let print_plan = {
            ...
            let plan = if visual_stack.contains(&span_id) {
                ...
                let indent_level = if let Some(parent_position) =
                    visual_stack.iter().position(|active_id| {
                        maybe_parent.is_some_and(|parent_id| *active_id == parent_id)
                    }) {
        ...
        if let Some(plan) = print_plan {
            ...
            Self::ignore_observer_error(Self::write_span_entry(
                &mut stderr,
                self.indent_amount,
                &plan,
            ));
    ...
    fn event(&self, event: &Event<'_>) -> SubscriberResult {
        ...
        let mut stderr = self.stderr.lock();
        let header_result = Self::write_event_header(
            &mut stderr,
            self.indent_amount,
            indent_level,
