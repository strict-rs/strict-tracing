  ...
  let (response_span, final_response) = match (req.method(), req.uri().path()) {
    ...
    (&Method::POST, "/echo/uppercase") => {
      ...
      *response.body_mut() = Full::new(Bytes::from(upper));
      (span!(Level::INFO, "response", response_kind = %"uppercase"), response)
    }
    ...
    (&Method::POST, "/echo/reversed") => {
      ...
      *response.body_mut() = Full::new(Bytes::from(reversed));
      (span!(Level::INFO, "reversed", body = ?response.body()), response)
    }
...
async fn main() -> Result<(), Error> {
  tracing_subscriber::fmt().with_max_level(Level::TRACE).try_init()?;
