...
- `NormalizeEvent` (impl'd for `Event`) lets subscribers recover full metadata from a log-sourced event: `is_log()` checks the callsite identity, and `normalized_metadata()` returns owned `NormalizedMetadata` (file/line/module/target) by replaying the event's fields through `LogVisitor`. Use `NormalizedMetadata::as_metadata()` when a short-lived `Metadata<'_>` view is required; do not fabricate lifetimes or reintroduce unsafe string casts.
...
- `interest_cache.rs` (the `interest-cache` feature) backs `InterestCacheConfig` (`Copy`/`Clone`) with a per-thread `ahash`/`lru` `LruCache` keyed on level+target, short-circuiting `LogTracer::enabled` for repeated callsites. A sentinel callsite bumps an epoch when `tracing-core` rebuilds interests so thread-local caches reset.
...
- `cargo nextest run -p tracing-log` runs the inline callsite tests plus integration tests `tests/log_tracer.rs` (asserts normalized metadata round-trips through a custom `Subscriber`; dev-dep enables `tracing-subscriber/env-filter`) and `tests/reexport_log_crate.rs` (confirms `log` is reachable as `tracing_log::log`).
