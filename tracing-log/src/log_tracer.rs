...
//!
//! * [`init`] if you want to convert all logs, regardless of log level, allowing the tracing
//!   `Subscriber` to perform any filtering
...
impl log::Log for LogTracer {
  fn enabled(&self, metadata: &log::Metadata<'_>) -> bool {
    ...
    let check_dispatch = || {
      ...
      dispatcher::get_default(|dispatch| dispatch.enabled(&metadata.as_trace()).unwrap_or(false))
    };
