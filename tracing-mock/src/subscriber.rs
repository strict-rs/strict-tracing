...
//!   .record(
//!     &span,
//!     expect::field("parting").with_value(&"goodbye world!"),
//!   )
//!   .exit(span)
//!   .only()
//!   .run_with_handle();
...
pub fn mock() -> MockSubscriber<DefaultFilter> {
  ...
  MockSubscriber {
    ...
    filter:    allow_all,
    max_level: None,
    name:      thread::current().name().unwrap_or("mock_subscriber").to_owned(),
  }
}
  ...
  F: Fn(&Metadata<'_>) -> bool + 'static,
{
  ...
  /// let (subscriber, handle) = subscriber::mock().enter(&span).exit(&span).only().run_with_handle();
  ///
  /// tracing::subscriber::with_default(subscriber, || {
  ...
  ///
  /// let span = expect::span().at_level(tracing::Level::INFO).named("the span we're testing");
  /// let (subscriber, handle) = subscriber::mock().enter(&span).exit(&span).run_with_handle();
  ///
  /// tracing::subscriber::with_default(subscriber, || {
  ///   let span = tracing::info_span!("the span we're testing");
  ///   let _entered = span.enter();
  ...
  ///
  /// let span = expect::span().at_level(tracing::Level::INFO).named("the span we're testing");
  /// let (subscriber, handle) = subscriber::mock().clone_span(span).run_with_handle();
  ///
  /// tracing::subscriber::with_default(subscriber, || {
  ///   let span = tracing::info_span!("the span we're testing");
  ///   _ = span.clone();
  ...
  ///
  /// let (subscriber, handle) = subscriber::mock().follows_from(consequence, cause).run_with_handle();
  ///
  /// tracing::subscriber::with_default(subscriber, || {
  ...
  {
    self.expected.push_back(Expect::Visit(span.into(), fields.into()));
    self
  }
  ...
  /// use tracing_mock::subscriber;
  ///
  /// let (subscriber, handle) = subscriber::mock().event(expect::event()).only().run_with_handle();
  ///
  /// tracing::subscriber::with_default(subscriber, || {
  ///   tracing::info!("a");
  ///   tracing::info!("b");
  ...
  F: Fn(&Metadata<'_>) -> bool + 'static,
{
  ...
  fn event(&self, event: &Event<'_>) -> SubscriberResult {
    ...
    match next {
      ...
      Some(Expect::Event(mut expected)) => {
        ...
        let _result = self.record_result(expected.check(event, event_get_ancestry, &self.name));
      }
      Some(ex) => {
  ...
  fn new_span(&self, span: &Attributes<'_>) -> SubscriberResult<Id> {
      ...
      .fetch_update(Ordering::SeqCst, Ordering::SeqCst, |id| id.checked_add(1))
    {
      Ok(id) => id,
      Err(id) => {
        self.record_failure(format_args!("[{}] span ID allocator exhausted at {}", self.name, id));
    ...
    let Some(id) = Id::try_from_u64(span_id_u64) else {
      self.record_failure(format_args!("[{}] span ID allocator produced zero", self.name));
      return Ok(Self::fallback_span_id());
    };
    let next_new_span = {
      let mut expected = self.expected.lock();
    ...
    if let Some(Expect::NewSpan(mut expected_span)) = next_new_span {
      ...
      let _result = self.record_result(expected_span.check(
        ...
        || {
          let spans = self.spans.lock();
          get_ancestry(&span, || self.lookup_current(), |span_id| spans.get(span_id).map(Into::into))
        },
        &self.name,
      ));
    }
    {
      ...
      let _previous = spans.insert(id, SpanState {
        id,
        name: meta.name(),
        refs: 1,
        meta,
  ...
  fn enter(&self, id: Id) -> SubscriberResult {
    let span_state = {
      let spans = self.spans.lock();
      spans.get(&id).map(|span| (span.name, ActualSpan::from(span)))
    };
    ...
    match next {
      ...
      Some(Expect::Enter(ref expected_span)) => {
        let _result = self.record_result(expected_span.check(&actual_span, "to enter a span", &self.name));
      }
      Some(ex) => {
        let _result = self.record_result(ex.bad(&self.name, format_args!("entered span `{span_name}`")));
      }
    }
    self.current.lock().push(id);
    Ok(())
  ...
  fn exit(&self, id: Id) -> SubscriberResult {
    ...
    match next {
      ...
      Some(ex) => {
        let _result = self.record_result(ex.bad(&self.name, format_args!("exited span `{span_name}`")));
      }
    }
    Ok(())
  }
  ...
  fn current_span(&self) -> SubscriberResult<Current> {
    ...
    Ok(current.map_or_else(Current::none, |id| {
      ...
      let Some(state) = spans.get(&id) else {
        self.record_failure(format_args!("[{}] no state for current span ID `{}`", self.name, id.into_u64()));
        return Current::none();
      };
