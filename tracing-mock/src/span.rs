...
//!
//! let span = expect::span().named("interesting_span").at_level(tracing::Level::INFO);
//!
...
impl ExpectedSpan {
  ...
  ///
  /// let (subscriber, handle) = subscriber::mock().enter(span).run_with_handle();
  ///
  /// tracing::subscriber::with_default(subscriber, || {
  ...
  ///
  /// If an span is record with a level that differs from the one provided to this method, the
  /// expectation will fail.
  ///
  /// # Examples
  ///
  ...
  /// use tracing_mock::subscriber;
  ///
  /// let span = expect::span().at_level(tracing::Level::INFO);
  ///
  /// let (subscriber, handle) = subscriber::mock().enter(span).run_with_handle();
  ///
  /// tracing::subscriber::with_default(subscriber, || {
  ...
  /// let span = expect::span().with_ancestry(expect::has_explicit_parent("parent_span"));
  ///
  /// let (subscriber, handle) = subscriber::mock()
  ///   .new_span(&parent_span)
  ///   .enter(&parent_span)
  ...
  /// use tracing_mock::subscriber;
  ///
  /// let span = expect::span().with_ancestry(expect::is_contextual_root());
  ///
  ...
  /// use tracing_mock::subscriber;
  ///
  /// let span = expect::span().with_fields(expect::field("field.name").with_value(&"field_value"));
  ///
  /// let (subscriber, handle) = subscriber::mock().new_span(span).run_with_handle();
  ///
  /// tracing::subscriber::with_default(subscriber, || {
  ...
  pub(crate) fn check(&self, actual: &ActualSpan, ctx: impl fmt::Display, subscriber_name: &str) -> ExpectationResult {
      ...
      None => {
        if self.metadata.has_expectations() {
          return Err(ExpectationError::from_args(format_args!(
            "[{subscriber_name}] expected {ctx} a span with valid metadata, but got one with unknown Id={actual_id}",
...
impl NewSpan {
  ...
  pub(crate) fn check(
    ...
    let name = meta.name();
    self.span.metadata.check(meta, "a new span", subscriber_name)?;
    ...
    if let Some(ref expected_ancestry) = self.ancestry {
      ...
      expected_ancestry.check(&actual_ancestry, format_args!("span `{name}`"), subscriber_name)?;
    }
...
impl ExpectedId {
  ...
  pub(crate) fn check(&self, actual: Id, ctx: fmt::Arguments<'_>, subscriber_name: &str) -> ExpectationResult {
...
impl fmt::Display for SetActualSpanIdError {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    write!(
      f,
      "Could not set `ExpecedId` to {new}, it had already been set to {previous}",
