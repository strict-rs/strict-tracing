...
mod tests {
  ...
  enum TestError {
    ...
    #[error(transparent)]
    Subscriber(#[from] strict_test_support::ResultFailure<SetGlobalDefaultError>),
  }
  ...
  #[test]
  fn log_with_trace() -> Result<(), TestError> {
    ensure_ok(set_global_default(NopSubscriber), "global tracing subscriber installs")?;
    ...
    trace_span.in_scope(|| -> Result<(), TestError> {
      test.try_assert_logged("-> foo; span=1")?;
