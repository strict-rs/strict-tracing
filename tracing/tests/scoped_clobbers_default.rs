...
mod tests {
  ...
  fn scoped_clobbers_global() -> Result<(), TestFailure> {
    ...
    let guard = set_default(scoped);
    tracing::info!("before global");

    // Now, set the global default.
    ensure_ok(set_global_default(global), "global default should not already be set")?;
    ...
    ensure_ok(scoped_handle.finished(), "mock expectations should finish")?;
    ensure_ok(global_handle.finished(), "mock expectations should finish")?;
    Ok(())
  }
}
