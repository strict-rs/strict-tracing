...
mod tests {
    ...
    // needs API breakage
    //event_without_message!{nonzerou128_event_without_message: std::num::NonZeroU128::new(42).unwrap_or(std::num::NonZeroU128::MIN)}
    ...
    fn move_field_out_of_struct() -> Result<(), TestFailure> {
        ...
        let (subscriber, handle) = subscriber::mock()
            ...
            .event(
                expect::event()
                    .with_fields(expect::field("position").with_value(&debug(&expected_pos))),
            )
            .only()
            .run_with_handle();
    ...
    fn string_field() -> Result<(), TestFailure> {
        let (subscriber, handle) = subscriber::mock()
            .event(
                expect::event().with_fields(expect::field("my_string").with_value(&"hello").only()),
            )
            .event(
                expect::event().with_fields(
