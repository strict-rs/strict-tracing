...
mod tests {
    ...
    fn handles_to_different_spans_with_the_same_metadata_are_not_equal() -> Result<(), TestFailure>
    {
        ...
        with_default(subscriber::mock().run(), || {
            ...
            let foo2 = make_span();
                ...
                foo1 != foo2,
                "different span instances with identical metadata are not equal",
            )
        })
    }
    ...
    -> Result<(), TestFailure> {
        ...
        .join()
        .map_or_else(
            |_panic| ensure(false, "subscriber thread should join"),
            |()| Ok(()),
        )
    }
    ...
    #[cfg_attr(target_arch = "wasm32", wasm_bindgen_test::wasm_bindgen_test)]
    #[test]
    fn clone_and_close_span_always_go_to_the_subscriber_that_tagged_the_span()
    -> Result<(), TestFailure> {
    ...
    fn move_field_out_of_struct() -> Result<(), TestFailure> {
        ...
        let (subscriber, handle) = subscriber::mock()
            ...
            .new_span(
                expect::span().named("bar").with_fields(
                    expect::field("position")
                        .with_value(&debug(&expected_pos))
                        .only(),
                ),
            )
            .run_with_handle();
    ...
    */
    ...
    fn keyword_ident_in_field_name_span_macro() -> Result<(), TestFailure> {
        ...
        let (subscriber, handle) = subscriber::mock()
            .new_span(
                expect::span().with_fields(expect::field("self").with_value(&debug(Foo)).only()),
            )
            .only()
            .run_with_handle();
