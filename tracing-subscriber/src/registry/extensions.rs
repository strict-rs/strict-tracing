...
impl ExtensionsInner {
  ...
  pub(super) fn remove<T: Send + Sync + 'static>(&mut self) -> Option<T> {
    self
      .map
      .remove(&TypeId::of::<T>())
      .and_then(|boxed| boxed.downcast().ok().map(|typed| *typed))
  }
...
mod tests {
  ...
  #[test]
  fn test_extensions() -> Result<(), ConditionFailure> {
    ...
    ensure(extensions.get() == Some(&5_i32), "inserted i32 is readable").map(drop)?;
    ensure(extensions.get_mut() == Some(&mut 5_i32), "inserted i32 is mutably readable").map(drop)?;

    ensure(extensions.remove::<i32>() == Some(5_i32), "inserted i32 is removable").map(drop)?;
    ensure(extensions.get::<i32>().is_none(), "removed i32 is no longer present").map(drop)?;

    ensure(extensions.get::<bool>().is_none(), "missing bool is not present").map(drop)?;
    ensure(extensions.get() == Some(&MyType(10)), "inserted custom type remains present").map(drop)
  }

  #[test]
  fn clear_retains_capacity() -> Result<(), ConditionFailure> {
    let mut extensions = ExtensionsInner::new();
    let _first_previous = extensions.insert(5_i32);
    let _second_previous = extensions.insert(MyType(10));
    let _third_previous = extensions.insert(true);

    ensure(extensions.map.len() == 3, "extensions store three entries").map(drop)?;
    let prev_capacity = extensions.map.capacity();
    extensions.clear();

    ensure(extensions.map.is_empty(), "after clear(), extensions map should have length 0").map(drop)?;
    ensure(
  ...
  #[test]
  fn clear_drops_elements() -> Result<(), ConditionFailure> {
    use std::sync::Arc;
    ...
    ensure(val1_dropped.upgrade().is_some(), "first value is live before clear").map(drop)?;
    ensure(val2_dropped.upgrade().is_some(), "second value is live before clear").map(drop)?;

    extensions.clear();
    ensure(val1_dropped.upgrade().is_none(), "after clear(), val1 should be dropped").map(drop)?;
    ensure(val2_dropped.upgrade().is_none(), "after clear(), val2 should be dropped").map(drop)
  }
}
