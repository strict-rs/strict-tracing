...
mod tests {
  ...
  #[test]
  fn pop_last_span() -> Result<(), ConditionFailure> {
    let mut stack = SpanStack::default();
    let span_id = FIRST_SPAN_ID;
    stack.push(span_id);

    ensure(stack.pop(span_id), "last span can be popped").map(drop)
  }

  #[test]
  fn pop_first_span() -> Result<(), ConditionFailure> {
    let mut stack = SpanStack::default();
    stack.push(FIRST_SPAN_ID);
    stack.push(SECOND_SPAN_ID);

    let span_id = FIRST_SPAN_ID;
    ensure(stack.pop(span_id), "first span can be popped").map(drop)
  }
}
