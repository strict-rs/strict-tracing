...
//! # }
//!
//! let subscriber = Registry::default().with(FooLayer::new()).with(BarLayer::new());
//! ```
//!
  ...
  R: LookupSpan<'a>,
{
  ...
  /// ```
  ///
  /// If the opposite order (from the root to this span) is desired, calling [`Scope::root_to_leaf`]
  /// on the returned iterator reverses the order.
  ///
  /// ```rust
