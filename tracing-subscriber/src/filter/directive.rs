...
impl<T> DirectiveSet<T> {
  ...
  #[cfg(all(feature = "std", feature = "env-filter", feature = "smallvec"))]
  /// Returns whether the set contains no directives.
  pub(crate) fn is_empty(&self) -> bool {
    self.directives.is_empty()
  }

  #[cfg(all(feature = "std", feature = "env-filter", not(feature = "smallvec")))]
  /// Returns whether the set contains no directives.
...
impl<T: Match + Ord> DirectiveSet<T> {
  ...
  pub(crate) fn directives_for<'a>(&'a self, metadata: &'a Metadata<'a>) -> impl Iterator<Item = &'a T> + 'a {
    self.directives().filter(move |directive| directive.cares_about(metadata))
  }
...
impl DirectiveSet<StaticDirective> {
  ...
  pub(crate) fn directives_for_target<'a>(&'a self, target: &'a str) -> impl Iterator<Item = &'a StaticDirective> + 'a {
    self.directives().filter(move |directive| directive.cares_about_target(target))
  }
}
...
impl StaticDirective {
  ...
  pub(in crate::filter) const fn new(target: Option<String>, field_names: Vec<String>, level: LevelFilter) -> Self {
    Self {
      target,
      field_names,
      level,
...
mod tests {
  ...
  fn parse_static(source: &str) -> Result<StaticDirective, TestError> {
    ensure_ok(source.parse::<StaticDirective>(), "static directive should parse").map_err(TestError::from)
  }

  #[test]
  fn directive_set_sorts_replaces_and_tracks_max_level() -> Result<(), TestError> {
    let mut directives = DirectiveSet::default();
    ensure(directives.iter().next().is_none(), "default set is empty").map(drop)?;
    ensure(directives.max_level == LevelFilter::OFF, "default max level starts off").map(drop)?;
  ...
  #[test]
  fn directive_set_matches_metadata_and_target_only_queries() -> Result<(), TestError> {
    ...
    .map(drop)?;
    ensure(!directives.enabled(&OTHER_TARGET_EVENT), "nonmatching target is rejected").map(drop)?;
    ensure(
  ...
  #[test]
  fn static_directive_parse_display_default_and_ordering_contracts() -> Result<(), TestError> {
    let bare_target = parse_static("app::module")?;
    ensure(bare_target.target.as_deref() == Some("app::module"), "bare target parses target").map(drop)?;
    ensure(bare_target.level == LevelFilter::TRACE, "bare target defaults to trace").map(drop)?;

    let bare_level = parse_static("warn")?;
    ensure(bare_level.target.is_none(), "bare level has no target").map(drop)?;
    ensure(bare_level.level == LevelFilter::WARN, "bare level parses the level").map(drop)?;
    ...
    ensure(default.target.is_none(), "default directive has no target").map(drop)?;
    ensure(default.field_names.is_empty(), "default directive has no field filters").map(drop)?;
    ensure(default.level == LevelFilter::ERROR, "default directive enables error").map(drop)?;

    let specific = parse_static("app::module=info")?;
    let general = parse_static("app=trace")?;
    ensure(specific < general, "longer target sorts ahead of shorter target").map(drop)?;
    ensure(
  ...
  #[test]
  fn static_directive_rejects_malformed_field_and_level_syntax() -> Result<(), TestError> {
    let too_many_equals = "app=info=debug".parse::<StaticDirective>();
    ensure(too_many_equals.is_err(), "directives reject more than one equals sign").map(drop)?;
    ...
    ensure(too_many_fields.is_err(), "directives reject more than one field-list opener").map(drop)?;
    ...
    ensure(unclosed_fields.is_err(), "directives reject unclosed field lists").map(drop)?;
  ...
  #[test]
  #[cfg(all(feature = "std", feature = "env-filter"))]
  fn parse_error_display_and_source_preserve_error_kinds() -> Result<(), TestError> {
    let generic = ParseError::new();
    ensure(generic.source().is_none(), "generic parse error has no source").map(drop)?;
    ensure(
    ...
    ensure(message.source().is_none(), "message parse error has no source").map(drop)?;
    ensure(
