...
pub use self::field::BadName as BadFieldName;
...
/// # Directives
///
/// A filter consists of one or more comma-separated directives which match on [`Span`]s and
/// [`Event`]s. Each directive may have a corresponding maximum verbosity [`level`] which
...
///
/// The directive syntax is similar to that of [`env_logger`]'s. At a high level, the syntax for
/// directives consists of several parts:
///
/// ```text
...
///
/// - `target` matches the event or span's target. In general, this is the module path and/or crate
///   name. Examples of targets `h2`, `tokio::net`, or `tide::server`. For more information on
///   targets, please refer to [`Metadata`]'s documentation.
/// - `span` matches on the span's name. If a `span` directive is provided alongside a `target`, the
///   `span` directive will match on spans _within_ the `target`.
/// - `field` matches on [fields] within spans. Field names can also be supplied without a `value`
///   and will match on any [`Span`] or [`Event`] that has a field with that name. For example:
///   `[span{field=\"value\"}]=debug`, `[{field}]=trace`.
/// - `value` matches on the value of a span's field. If a value is a numeric literal or a bool, it
///   will match _only_ on that value. Otherwise, this filter matches the [`std::fmt::Debug`] output
///   from the value.
...
///     - The sole exception are the `field` and `value` directives. If a `value` is provided, a
///       `field` must _also_ be provided. However, the converse does not hold, as fields can be
///       matched without a value.
/// - If only a level is provided, it will set the maximum level for all `Span`s and `Event`s that
///   are not enabled by other filters.
/// - A directive without a level will enable anything that it matches. This is equivalent to
///   `=trace`.
/// - When a crate has a dash in its name, the default target for events will be the crate's module
///   path as it appears in Rust. This means every dash will be replaced with an underscore.
/// - A dash in a target will only appear when being specified explicitly: `tracing::info!(target:
///   "target-name", ...);`
///
/// ## Example Syntax
///
...
impl EnvFilter {
  ...
  fn enabled_for<S>(&self, metadata: &Metadata<'_>, _: Context<'_, S>) -> bool {
    ...
    if self.has_dynamics && self.dynamics.max_level >= *level {
      let enabled_by_callsite = metadata.is_span() && try_lock!(self.by_cs.read(), else false).contains_key(&metadata.callsite());
...
mod tests {
  ...
  enum TestError {
    ...
    #[error(transparent)]
    OptionDirectiveParseError(#[from] strict_test_support::OptionFailure<ParseError>),
    /// Preserves the complete native failure and its inputs.
    #[error(transparent)]
  ...
  fn ensure_callsite_interest(
    ...
    let interest = ensure_ok(filter.register_callsite(metadata), "callsite registration succeeds")?;
    let enabled = if expect_enabled {
      interest.is_always()
    } else {
      interest.is_never()
    };
    ensure(enabled, message).map(drop).map_err(TestError::from)
  }

  #[test]
  fn callsite_enabled_no_span_directive() -> Result<(), TestError> {
    ensure_callsite_interest(&SPAN_META_TRACE, "app=debug", false, "span directive should not enable callsite")
  }

  #[test]
  fn callsite_off() -> Result<(), TestError> {
    ensure_callsite_interest(&SPAN_META_ERROR, "app=off", false, "off directive should disable callsite")
  }

  #[test]
  fn callsite_enabled_includes_span_directive() -> Result<(), TestError> {
  ...
  #[test]
  fn callsite_enabled_includes_span_directive_field() -> Result<(), TestError> {
    ensure_callsite_interest(
  ...
  #[test]
  fn callsite_enabled_includes_span_directive_multiple_fields() -> Result<(), TestError> {
    ensure_callsite_interest(
      &SPAN_META_TRACE_FIELD,
  ...
  #[test]
  fn roundtrip() -> Result<(), TestError> {
    let first_filter: EnvFilter = ensure_ok(
      "[span1{foo=1}]=error,[span2{bar=2 baz=false}],crate2[{quux=\"quuux\"}]=debug".parse(),
      "source env filter should parse",
    )?;
    let second_filter: EnvFilter = ensure_ok(format!("{first_filter}").parse(), "formatted env filter should parse")?;
    ensure(first_filter.statics == second_filter.statics, "static directives roundtrip").map(drop)?;
    ensure(first_filter.dynamics == second_filter.dynamics, "dynamic directives roundtrip")
      .map(drop)
      .map_err(TestError::from)
  }

  #[test]
  fn size_of_filters() -> Result<(), TestError> {
    fn ensure_filter_has_size(source: &str) -> Result<(), TestError> {
    ...
    ensure_filter_has_size("[span1{foo=1}]=error,[span2{bar=2 baz=false}],crate2[{quux=\"quuux\"}]=debug")?;

    ensure_filter_has_size(
      "crate1::mod1=error,crate1::mod2=warn,crate1::mod2::mod3=info,crate2=debug,crate3=trace,crate3::mod2::mod1=off,[span1{foo=1}]=error,\
       [span2{bar=2 baz=false}],crate2[{quux=\"quuux\"}]=debug",
    )
  }

  #[test]
  fn parse_empty_string() -> Result<(), TestError> {
    ...
    ensure(EnvFilter::builder().parse("").is_ok(), "empty env filter should parse")
      .map(drop)
      .map_err(TestError::from)
  }

  #[test]
  fn constructors_apply_lossy_and_strict_parse_contracts() -> Result<(), TestError> {
    let lossy = EnvFilter::new("app=info,broken[");
    ensure_eq(
      String::from(lossy.to_string().as_str()),
      String::from("app=info"),
    ...
    let from_str = ensure_ok("app=warn".parse::<EnvFilter>(), "FromStr delegates to strict parsing")?;
    ensure_eq(
      String::from(from_str.to_string().as_str()),
    ...
    ensure_eq(
      String::from(default_filter.to_string().as_str()),
      String::new(),
  ...
  #[test]
  fn default_directive_is_used_when_lossy_input_has_no_valid_directives() -> Result<(), TestError> {
    let defaulted = EnvFilter::new("broken[");

    ensure_eq(
      String::from(defaulted.to_string().as_str()),
      String::from("error"),
  ...
  #[test]
  fn from_env_errors_preserve_parse_and_environment_sources() -> Result<(), TestError> {
    let parse_error = ensure_some(EnvFilter::try_new("broken[").err(), "parse error is available")?;
    ...
    .map(drop)?;
    ensure(parse_env_error.source().is_some(), "parse-backed environment error exposes source").map(drop)?;

    let missing_env_error = FromEnvError::from(env::VarError::NotPresent);
    ensure_eq(
      String::from(missing_env_error.to_string().as_str()),
      String::from("environment variable not found"),
      "environment-backed error displays variable failure",
    )
    .map(drop)?;
    ensure(missing_env_error.source().is_some(), "environment-backed error exposes source")
      .map(drop)
      .map_err(TestError::from)
  }

  #[test]
  fn static_dynamic_and_value_directives_report_level_hints() -> Result<(), TestError> {
