...
/// code outside of `tracing-subscriber`. In the future, new APIs will be added to
/// `tracing-subscriber` to allow non-Registry [`Subscriber`]s to also participate in per-layer
...
///
/// 1. When filtering a span, the Registry must store the `FilterMap` generated by
///    `Filtered::enabled` calls for that span as part of the span's per-span data. This allows
///    `Filtered` layers to determine whether they had previously disabled a given span, and avoid
///    showing it to the wrapped layer if it was disabled.
///
...
/// 2. If all the bits are set, then every per-layer filter has decided it doesn't want to enable
///    that span or event. In that case, the `Registry`'s `enabled` method will return `false`, so
///    that recording a span or event can be skipped entirely.
#[derive(Debug)]
...
pub trait FilterExt<S>: layer::Filter<S> {
  ...
  /// // `my_crate`:
  /// let my_crate = filter_fn(|meta| meta.target().starts_with("my_crate"));
  ///
  /// let filter = my_crate
...
macro_rules! filter_impl_body {
  () => {
    ...
    #[inline]
    fn on_record(&self, id: span::Id, values: &span::Record<'_>, ctx: Context<'_, S>) -> SubscriberResult<()> {
  ...
  F: layer::Filter<S>,
{
  ...
  fn enabled(&self, meta: &Metadata<'_>, ctx: &Context<'_, S>) -> SubscriberResult<bool> {
    self.as_ref().map_or(Ok(true), |inner| inner.enabled(meta, ctx))
  }
  ...
  #[inline]
  fn max_level_hint(&self) -> SubscriberResult<Option<LevelFilter>> {
    self.as_ref().map_or(Ok(None), layer::Filter::max_level_hint)
  }
  ...
  L: Layer<S>,
{
  ...
  fn on_new_span(&self, attrs: &span::Attributes<'_>, id: span::Id, cx: Context<'_, S>) -> SubscriberResult<()> {
    self.did_enable(|| {
      let filtered_cx = cx.with_filter(self.id());
  ...
  fn on_record(&self, span: span::Id, values: &span::Record<'_>, cx: Context<'_, S>) -> SubscriberResult<()> {
  ...
  fn on_follows_from(&self, span: span::Id, follows: span::Id, cx: Context<'_, S>) -> SubscriberResult<()> {
    ...
    if cx.is_enabled_for(span, self.id()) && cx.is_enabled_for(follows, self.id()) {
      self.layer.on_follows_from(span, follows, cx.with_filter(self.id()))?;
    }
    Ok(())
  }
  ...
  fn event_enabled(&self, event: &Event<'_>, cx: Context<'_, S>) -> SubscriberResult<bool> {
    let filtered_cx = cx.with_filter(self.id());
    let enabled = FILTERING.with(|filtering| filtering.and(self.id(), || self.filter.event_enabled(event, &filtered_cx)))?;
  ...
  fn on_id_change(&self, old: span::Id, new: span::Id, cx: Context<'_, S>) -> SubscriberResult<()> {
...
impl FilterId {
  ...
  pub(crate) fn new(id: u8) -> Self {
    1_u64.checked_shl(u32::from(id)).map_or_else(Self::disabled, Self)
  }
...
impl fmt::Debug for FilterId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    if self.0 == Self::disabled().0 {
      return f.debug_tuple("FilterId").field(&format_args!("DISABLED")).finish();
    }
...
impl fmt::Binary for FilterId {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    f.debug_tuple("FilterId").field(&format_args!("{:b}", self.0)).finish()
  }
}
...
impl FilterState {
  ...
  fn add_interest(&self, interest: Interest) {
    ...
    if (existing_interest.is_always() && !interest.is_always()) || (existing_interest.is_never() && !interest.is_never()) {
  ...
  /// `Filtered`.
  fn did_enable(&self, filter: FilterId, f: impl FnOnce() -> SubscriberResult<()>) -> SubscriberResult<()> {
  ...
  fn and(&self, filter: FilterId, f: impl FnOnce() -> SubscriberResult<bool>) -> SubscriberResult<bool> {
    let map = self.enabled.get();
...
where
  ...
  S: Subscriber,
{
  layer.downcast_ref_by_id(TypeId::of::<MagicPlfDowncastMarker>()).is_some()
}
...
mod tests {
  ...
  enum TestError {
    ...
    #[error(transparent)]
    OptionInterest(#[from] strict_test_support::OptionFailure<Interest>),
    /// Preserves the complete native failure and its inputs.
    #[error(transparent)]
    OptionSpanId(#[from] strict_test_support::OptionFailure<Id>),
    /// Preserves the complete native failure and its inputs.
    #[error(transparent)]
    ResultSubscriberError(#[from] strict_test_support::ResultFailure<SubscriberError>),
    /// Preserves the complete native failure and its inputs.
    #[error(transparent)]
    ResultCellBorrowError(#[from] strict_test_support::ResultFailure<cell::BorrowError>),
  ...
  impl<S> layer::Filter<S> for RecordingFilter {
    ...
    fn on_new_span(&self, _attrs: &Attributes<'_>, _id: Id, _ctx: Context<'_, S>) -> SubscriberResult<()> {
      self.record("on_new_span");
      Ok(())
    }

    fn on_record(&self, _id: Id, _values: &Record<'_>, _ctx: Context<'_, S>) -> SubscriberResult<()> {
  ...
  fn with_instrumentation_fixtures<R>(
    f: impl FnOnce(&Event<'_>, &Attributes<'_>, &Record<'_>, Id) -> Result<R, TestError>,
  ) -> Result<R, TestError> {
  ...
  fn exercise_forwarding_filter<F>(filter: &F, calls: &CallLog, label: &'static str) -> Result<(), TestError>
  where
    F: layer::Filter<registry::Registry>,
  {
    with_instrumentation_fixtures(|event, attrs, record, span_id| {
      ...
      let expected = vec![
        "enabled", "callsite_enabled", "max_level_hint", "event_enabled", "on_new_span", "on_record", "on_enter", "on_exit", "on_close",
      ];
  ...
  #[test]
  fn filter_id_and_filter_map_track_disabled_filters() -> Result<(), TestError> {
    ...
    .map(drop)?;
    ensure(first.and(second).0 == 3, "distinct filter IDs combine into a shared mask").map(drop)?;

    let disabled_debug = format!("{:?}", FilterId::disabled());
    ensure_contains(disabled_debug, String::from("DISABLED"), "disabled debug is named explicitly").map(drop)?;
    let first_binary = format!("{first:b}");
    ensure_contains(first_binary, String::from("FilterId"), "binary formatting names the ID type").map(drop)?;
    let first_alternate = format!("{first:#?}");
    ensure_contains(first_alternate, String::from("bits"), "alternate debug includes raw bits").map(drop)?;

    let mut map = FilterMap::new();
    ensure(map.is_enabled(first), "fresh maps treat the first filter as enabled").map(drop)?;
    ensure(map.is_enabled(second), "fresh maps treat the second filter as enabled").map(drop)?;
    ensure(map.any_enabled(), "fresh maps have at least one enabled filter").map(drop)?;

    map = map.set(first, false);
    ensure(!map.is_enabled(first), "disabling a filter records its bit").map(drop)?;
    ensure(map.is_enabled(second), "disabling one filter leaves another enabled").map(drop)?;
    ensure(map.any_enabled(), "one disabled filter does not disable the whole stack").map(drop)?;

    map = map.set(first, true);
    ensure(map.is_enabled(first), "re-enabling a filter clears its bit").map(drop)?;
    let before_disabled_sentinel = map;
    map = map.set(FilterId::disabled(), false);
    ensure(map == before_disabled_sentinel, "disabled sentinel does not change the map").map(drop)?;
    let map_debug = format!("{map:?}");
    ensure_contains(map_debug, String::from("FilterMap"), "map debug names the type").map(drop)?;
    let map_alternate = format!("{map:#?}");
    ensure_contains(map_alternate, String::from("bits"), "alternate map debug includes raw bits").map(drop)?;
    let map_binary = format!("{map:b}");
    ensure_contains(map_binary, String::from("bits"), "binary map formatting includes raw bits")
      .map(drop)
      .map_err(TestError::from)
  }

  #[test]
  fn filter_state_combines_interests_and_consumes_disabled_callbacks() -> Result<(), TestError> {
    ...
    ensure(!callback_ran, "disabled filters do not run callbacks").map(drop)?;
    ensure(
    ...
    ensure(!disabled, "second pass can disable an enabled filter").map(drop)?;
    ensure(
      !state.filter_map().is_enabled(filter),
    ...
    ensure(!disabled_again, "previously disabled filters stay disabled").map(drop)?;
    ensure(skipped_second_pass, "disabled filters skip later second-pass callbacks").map(drop)?;

    state.set(filter, true);
    let enabled = ensure_ok(state.and(filter, || Ok(true)), "re-enabled second pass returns")?;
    ensure(enabled, "re-enabled filters can pass the second pass").map(drop)?;
    ensure(
      state.filter_map().is_enabled(filter),
  ...
  #[test]
  fn thread_local_filter_state_reports_and_clears_global_disablement() -> Result<(), TestError> {
    FILTERING.with(|filtering| {
      filtering.enabled.set(FilterMap {
        bits: u64::MAX
      });
    });
    ensure(
    ...
    let interest = ensure_some(FilterState::take_interest(), "thread-local interest is available to the registry")?;
    ensure(interest.is_sometimes(), "thread-local interest is preserved").map(drop)?;
    ensure(FilterState::take_interest().is_none(), "taking thread-local interest consumes it").map(drop)?;
  ...
  #[test]
  fn filtered_accessors_debug_and_downcast_expose_public_parts() -> Result<(), TestError> {
    type TestFiltered = Filtered<AccessLayer, LevelFilter, registry::Registry>;

    let mut filtered = TestFiltered::new(
      AccessLayer {
        name: "inner"
      },
      LevelFilter::INFO,
    ...
    ensure_eq(
      String::from(filtered.inner().name),
      String::from("inner"),
      "inner accessor exposes the layer",
    )
    .map(drop)?;
    ensure_eq(*filtered.filter(), LevelFilter::INFO, "filter accessor exposes the filter").map(drop)?;
    *filtered.filter_mut() = LevelFilter::WARN;
    ensure_eq(*filtered.filter(), LevelFilter::WARN, "filter_mut updates the filter").map(drop)?;
    filtered.inner_mut().name = "changed";
    ensure_eq(
      String::from(filtered.inner().name),
      String::from("changed"),
      "inner_mut updates the layer",
    )
    .map(drop)?;

    let debug = format!("{filtered:?}");
    ensure_contains(debug.clone(), String::from("Filtered"), "debug output names the wrapper").map(drop)?;
    ensure_contains(debug.clone(), String::from("filter"), "debug output includes the filter").map(drop)?;
    ensure_contains(debug, String::from("layer"), "debug output includes the inner layer").map(drop)?;
    ...
    .map(drop)?;
      ...
      Layer::<registry::Registry>::downcast_ref_by_id(&filtered, TypeId::of::<MagicPlfDowncastMarker>()).is_some(),
  ...
  #[test]
  fn box_arc_and_some_option_filters_forward_to_inner_filter() -> Result<(), TestError> {
    let boxed_calls = Arc::new(Mutex::new(Vec::new()));
    let boxed: Box<dyn layer::Filter<registry::Registry> + Send + Sync> = Box::new(RecordingFilter::new(Arc::clone(&boxed_calls)));
    exercise_forwarding_filter(&boxed, &boxed_calls, "boxed filter forwards")?;

    let arc_calls = Arc::new(Mutex::new(Vec::new()));
    let arc: Arc<dyn layer::Filter<registry::Registry> + Send + Sync> = Arc::new(RecordingFilter::new(Arc::clone(&arc_calls)));
  ...
  #[test]
  fn none_option_filter_allows_without_inner_filter() -> Result<(), TestError> {
    ...
    with_instrumentation_fixtures(|event, attrs, record, span_id| {
      ...
      ensure_ok(filter.on_close(span_id, context), "none option close").map_err(TestError::from)
    })
  }
}
