  ...
  B: Filter<S>,
{
  ...
  fn max_level_hint(&self) -> SubscriberResult<Option<LevelFilter>> {
    ...
    Ok(cmp::min(self.left.max_level_hint()?, self.right.max_level_hint()?))
  }

  #[inline]
  fn event_enabled(&self, event: &tracing_core::Event<'_>, cx: &Context<'_, S>) -> SubscriberResult<bool> {
  ...
  A: Filter<S>,
{
  ...
  #[inline]
  fn event_enabled(&self, _event: &tracing_core::Event<'_>, _cx: &Context<'_, S>) -> SubscriberResult<bool> {
...
mod tests {
    ...
    F: Filter<()>,
  {
    ...
    ensure_ok(filter.on_close(span_id, context.clone()), label).map_err(TestError::from)
  }

  #[test]
  fn boolean_combinators_short_circuit_enabled_and_event_enabled() -> Result<(), TestError> {
    let calls = call_log();
    ...
    let and_events = And::new(
      ...
      !ensure_ok(and_events.event_enabled(&event, &context), "and event_enabled returns")?,
  ...
  #[test]
  fn callsite_interest_and_hint_algebra_matches_combinator_contracts() -> Result<(), TestError> {
    let calls = call_log();
    let and_never: And<_, _, ()> = And::new(
      ...
      ensure_ok(and_never.callsite_enabled(&COMBINATOR_META), "and never interest returns")?.is_never(),
      "and callsite interest short-circuits never",
    )
    .map(drop)?;
    ensure(calls.lock().as_slice() == ["left.callsite"], "and callsite never avoids right side").map(drop)?;
    ...
    let and_hint: And<_, _, ()> = And::new(
      ...
      ensure_ok(and_hint.callsite_enabled(&COMBINATOR_META), "and sometimes interest returns")?.is_sometimes(),
      "and keeps left interest when right is always",
    )
    .map(drop)?;
    ensure(
      ensure_ok(and_hint.max_level_hint(), "and max-level hint returns")? == Some(LevelFilter::INFO),
    ...
    let or_hint: Or<_, _, ()> = Or::new(
      ...
      ensure_ok(or_hint.callsite_enabled(&COMBINATOR_META), "or interest returns")?.is_sometimes(),
      "or returns sometimes when either side is sometimes",
    )
    .map(drop)?;
    ensure(
      ensure_ok(or_hint.max_level_hint(), "or max-level hint returns")? == Some(LevelFilter::DEBUG),
    ...
    let or_unknown_hint: Or<_, _, ()> = Or::new(
      ...
      ensure_ok(or_unknown_hint.max_level_hint(), "or unknown max-level hint returns")?.is_none(),
  ...
  #[test]
  fn not_inverts_static_interest_and_metadata_enabled_only() -> Result<(), TestError> {
    let calls = call_log();
    let context = Context::<()>::none();
    let value_set = COMBINATOR_META.fields().value_set(&[]);
    let event = Event::new(&COMBINATOR_META, &value_set);
    ...
    .map(drop)?;
      ...
      ensure_ok(filter.callsite_enabled(&COMBINATOR_META), "not interest returns")?.is_never(),
  ...
  #[test]
  fn and_lifecycle_hooks_forward_to_both_sides() -> Result<(), TestError> {
    let calls = call_log();
    let context = Context::<()>::none();
    let value_set = COMBINATOR_META.fields().value_set(&[]);
    ...
    run_lifecycle_hooks(&filter, &attrs, &record, span_id, &context, "and forwards hooks")?;
      ...
      calls.lock().as_slice()
        == [
          "left.on_new_span", "right.on_new_span", "left.on_record", "right.on_record", "left.on_enter", "right.on_enter", "left.on_exit",
          "right.on_exit", "left.on_close", "right.on_close",
        ],
  ...
  #[test]
  fn or_lifecycle_hooks_forward_to_both_sides() -> Result<(), TestError> {
    let calls = call_log();
    let context = Context::<()>::none();
    let value_set = COMBINATOR_META.fields().value_set(&[]);
    let attrs = Attributes::new(&COMBINATOR_META, &value_set);
  ...
  #[test]
  fn not_lifecycle_hooks_forward_to_the_inner_filter() -> Result<(), TestError> {
    let calls = call_log();
    let context = Context::<()>::none();
    let value_set = COMBINATOR_META.fields().value_set(&[]);
    let attrs = Attributes::new(&COMBINATOR_META, &value_set);
    ...
    run_lifecycle_hooks(&filter, &attrs, &record, span_id, &context, "not forwards hooks")?;
      ...
      calls.lock().as_slice()
        == [
          "inner.on_new_span", "inner.on_record", "inner.on_enter", "inner.on_exit", "inner.on_close",
        ],
