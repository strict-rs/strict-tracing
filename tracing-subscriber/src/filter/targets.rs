...
impl Targets {
  ...
  pub fn with_target(mut self, target: impl Into<String>, level: impl Into<LevelFilter>) -> Self {
    self
      .0
      .add(StaticDirective::new(Some(target.into()), Vec::default(), level.into()));
    self
  }
  ...
  pub fn with_default(mut self, level: impl Into<LevelFilter>) -> Self {
    self.0.add(StaticDirective::new(None, Vec::default(), level.into()));
    self
  }
  ...
  L: Into<LevelFilter>,
{
  fn extend<I: IntoIterator<Item = (T, L)>>(&mut self, directives: I) {
    let parsed_directives = directives
      .into_iter()
      .map(|(target, level)| StaticDirective::new(Some(target.into()), Vec::default(), level.into()));
...
impl FromStr for Targets {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    s.split(',').map(StaticDirective::from_str).collect::<Result<_, _>>().map(Self)
  }
}
  ...
  fn into_iter(self) -> Self::IntoIter {
    Iter(
      self
        .0
        .iter()
        .filter_map(|directive| directive.target.as_deref().map(|target| (target, directive.level))),
    )
  }
}
...
type IntoTargetIter =
  FilterMap<<DirectiveSet<StaticDirective> as IntoIterator>::IntoIter, fn(StaticDirective) -> Option<(String, LevelFilter)>>;
...
type TargetIter<'a> = FilterMap<slice::Iter<'a, StaticDirective>, fn(&'a StaticDirective) -> Option<(&'a str, LevelFilter)>>;
...
mod tests {
  ...
  fn expect_parse(source: &str) -> Result<Targets, TestError> {
    ensure_ok(source.parse::<Targets>(), "target filter should parse").map_err(TestError::from)
  }

  fn ensure_targets(dirs: &[StaticDirective], expected: &[TargetExpectation]) -> Result<(), TestError> {
    ensure(dirs.len() == expected.len(), "parsed target count matches").map(drop)?;
    for (directive, expectation) in dirs.iter().zip(expected) {
      ...
      .map(drop)?;
      ensure(directive.level == expectation.level, "parsed target level matches").map(drop)?;
      ensure(directive.field_names.is_empty(), "parsed target has no field filters").map(drop)?;
    }
    Ok(())
  }

  fn expect_parse_ralith(source: &str) -> Result<(), TestError> {
  ...
  fn expect_parse_level_directives(source: &str) -> Result<(), TestError> {
    let dirs = expect_parse(source)?.0.into_vec();
    ensure_targets(&dirs, &[
      TargetExpectation {
  ...
  #[test]
  fn parse_ralith() -> Result<(), TestError> {
    expect_parse_ralith("common=info,server=debug")
  }

  #[test]
  fn parse_ralith_uc() -> Result<(), TestError> {
    expect_parse_ralith("common=INFO,server=DEBUG")
  }

  #[test]
  fn parse_ralith_mixed() -> Result<(), TestError> {
    let _targets = expect_parse("common=iNfo,server=dEbUg")?;
    Ok(())
  }

  #[test]
  fn expect_parse_valid() -> Result<(), TestError> {
  ...
  fn parse_numeric_level_directives() -> Result<(), TestError> {
    expect_parse_level_directives("crate1::mod1=1,crate1::mod2=2,crate1::mod2::mod3=3,crate2=4,crate3=5,crate3::mod2::mod1=0")
  }

  #[test]
  fn targets_iter() -> Result<(), TestError> {
    let filter = expect_parse("crate1::mod1=error,crate1::mod2,crate2=debug,crate3=off")?.with_default(LevelFilter::WARN);
  ...
  #[test]
  fn targets_into_iter() -> Result<(), TestError> {
    let filter = expect_parse("crate1::mod1=error,crate1::mod2,crate2=debug,crate3=off")?.with_default(LevelFilter::WARN);
  ...
  #[test]
  fn targets_builders_iterators_and_would_enable_preserve_static_contracts() -> Result<(), TestError> {
    let empty = Targets::new();
    ensure(empty.to_string().is_empty(), "empty targets display as an empty directive list").map(drop)?;
    ensure(
    ...
    let built = Targets::new().with_targets(vec![("app", LevelFilter::INFO), ("app::module", LevelFilter::DEBUG)]);
    ensure(
    ...
    let collected = Targets::from_iter(vec![(String::from("collected"), LevelFilter::WARN)]);
  ...
  #[test]
  fn targets_layer_and_filter_hooks_return_static_interests() -> Result<(), TestError> {
    ...
    .map(drop)?;
      ...
      ensure_ok(layer::Layer::<NoSubscriber>::max_level_hint(&filter), "layer max-level hint")? == Some(LevelFilter::INFO),
    ...
    #[cfg(feature = "registry")]
    {
      ...
      .map(drop)?;
        ...
        ensure_ok(layer::Filter::<NoSubscriber>::max_level_hint(&filter), "filter max-level hint")? == Some(LevelFilter::INFO),
  ...
  #[test]
  fn targets_default_level() -> Result<(), TestError> {
    let initial_filter = expect_parse("crate1::mod1=error,crate1::mod2,crate2=debug,crate3=off")?;
    ensure(initial_filter.default_level().is_none(), "default level starts empty").map(drop)?;

    let off_filter = expect_parse("crate1::mod1=error,crate1::mod2,crate2=debug,crate3=off")?.with_default(LevelFilter::OFF);
    ensure(
  ...
  fn size_of_filters() -> Result<(), TestError> {
    ...
    ensure_filter_has_size("crate1::mod1=error,crate1::mod2=warn,crate1::mod2::mod3=info,crate2=debug,crate3=trace,crate3::mod2::mod1=off")
  }
  ...
  #[test]
  fn display_roundtrips() -> Result<(), TestError> {
    fn test_roundtrip(source: &str) -> Result<(), TestError> {
      let filter = expect_parse(source)?;
      ...
      let parsed_filter = ensure_ok(formatted.parse::<Targets>(), "formatted target filter should parse")?;
    ...
    test_roundtrip("crate1::mod1=ERROR,crate1::mod2=WARN,crate1::mod2::mod3=INFO,crate2=DEBUG,crate3=TRACE,crate3::mod2::mod1=OFF")?;
    test_roundtrip("crate1::mod1=error,crate1::mod2=warn,crate1::mod2::mod3=info,crate2=debug,crate3=trace,crate3::mod2::mod1=off")?;
