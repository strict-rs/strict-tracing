...
use crate::filter::directive::DirectiveSet;
use crate::filter::directive::FilterVec;
use crate::filter::directive::Match;
use crate::filter::directive::ParseError;
use crate::filter::directive::StaticDirective;
use crate::filter::env::FieldMap;
use crate::filter::env::field;
...
impl Directive {
  ...
  pub(super) fn make_tables(directives: impl IntoIterator<Item = Self>) -> (Dynamics, Statics) {
    // TODO(eliza): this could be made more efficient...
    let (dyns, stats): (Vec<Self>, Vec<Self>) = directives.into_iter().partition(Self::is_dynamic);
    let statics = stats
      .into_iter()
  ...
  pub(super) fn deregexify(&mut self) {
    for field_match in &mut self.fields {
      field_match.value = match field_match.value.take() {
        Some(field::ValueMatch::Pat(pat)) => Some(field::ValueMatch::Debug(Box::new(pat.into_debug_match()))),
        existing => existing,
      }
    }
  }
  ...
  pub(super) fn parse(from: &str, regex: bool) -> Result<Self, ParseError> {
    ...
    for (position, token) in from.trim().char_indices() {
      parse_state = transition_parse_state(&mut directive, parse_state, from, position, token, regex)?;
    }
...
fn after_char(position: usize, token: char) -> Result<usize, ParseError> {
  position.checked_add(token.len_utf8()).ok_or_else(ParseError::new)
}

/// Returns a directive substring known by the parser state.
fn directive_part(source: &str, start: usize, end: usize) -> Result<&str, ParseError> {
...
fn parse_level_or_target(level_or_target: &str) -> Result<(LevelFilter, Option<String>), ParseError> {
...
fn push_field_match(directive: &mut Directive, source: &str, start: usize, end: usize, regex: bool) -> Result<(), ParseError> {
...
fn transition_start_state(position: usize, token: char) -> Result<DirectiveParseState, ParseError> {
  ...
  Ok(DirectiveParseState::LevelOrTarget {
    start: position
  })
}
...
fn transition_level_or_target_state(
  ...
  match token {
    ...
    _ => Ok(DirectiveParseState::LevelOrTarget {
      start,
    }),
  }
}
...
fn transition_field_state(
  ...
  match token {
    ...
    _ => Ok(DirectiveParseState::Field {
      field_start,
    }),
  }
}
...
fn transition_parse_state(
  ...
  use DirectiveParseState::Complete;
  use DirectiveParseState::Field;
  use DirectiveParseState::Fields;
  use DirectiveParseState::Level;
  use DirectiveParseState::LevelOrTarget;
  use DirectiveParseState::Span;
  use DirectiveParseState::Start;
  use DirectiveParseState::Target;

  match parse_state {
    Start => transition_start_state(position, token),
    LevelOrTarget {
      start,
    } => transition_level_or_target_state(directive, start, source, position, token),
    Target => transition_target_state(position, token),
    Span {
      span_start,
    } => transition_span_state(directive, span_start, source, position, token),
    Field {
      field_start,
    } => transition_field_state(directive, field_start, source, position, token, regex),
    Fields => transition_fields_state(token),
    Level {
      level_start,
    } => transition_level_state(directive, level_start, source, position, token),
...
fn finish_parse_state(directive: &mut Directive, parse_state: DirectiveParseState, source: &str) -> Result<(), ParseError> {
  match parse_state {
    DirectiveParseState::LevelOrTarget {
      start,
    } => {
      let (level, target) = parse_level_or_target(directive_part_from(source, start)?)?;
      directive.level = level;
...
mod test {
  ...
  enum TestError {
    ...
    #[error(transparent)]
    Parse(#[from] strict_test_support::ResultFailure<ParseError>),
  }
  ...
  fn expect_parse(dirs: impl AsRef<str>) -> Result<Vec<Directive>, TestError> {
    split_directives(dirs.as_ref())
      .map(|directive| ensure_ok(directive.parse::<Directive>(), "directive should parse").map_err(TestError::from))
      .collect()
  }

  fn ensure_directives(dirs: &[Directive], expected: &[DirectiveExpectation]) -> Result<(), TestError> {
    ensure_eq(dirs.len(), expected.len(), "parsed directive count").map(drop)?;
    for (directive, expectation) in dirs.iter().zip(expected) {
      ensure(directive.target.as_deref() == expectation.target, "parsed directive target matches").map(drop)?;
      ensure(directive.level == expectation.level, "parsed directive level matches").map(drop)?;
      ensure(directive.in_span.as_deref() == expectation.in_span, "parsed directive span matches").map(drop)?;
    }
    Ok(())
  }

  /// Parses `input`, sorts the directives, and checks the resulting target ordering.
  fn ensure_directive_target_order(input: &str, expected: &[&str], message: &'static str) -> Result<(), TestError> {
    let mut dirs = expect_parse(input)?;
    dirs.sort_unstable();
    let sorted = dirs.iter().map(|directive| directive.target.as_deref()).collect::<Vec<_>>();
    let expected_targets = expected.iter().copied().map(Some).collect::<Vec<_>>();
    ensure(sorted == expected_targets, message).map(drop).map_err(TestError::from)
  }

  /// Parses `input` and checks it yields the six-entry crate/level directive expectation.
  fn ensure_parses_level_directives(input: &str) -> Result<(), TestError> {
    let dirs = parse_directives(input);
    ensure_directives(&dirs, &[
  ...
  fn ensure_parses_ralith_case_insensitive(input: &str) -> Result<(), TestError> {
    let dirs = parse_directives(input);
    ensure_directives(&dirs, &[
      DirectiveExpectation {
        target:  Some("common"),
  ...
  #[test]
  fn directive_ordering_by_target_len() -> Result<(), TestError> {
    ...
    ensure_directive_target_order(
      ...
      &["a_really_long_name_with_no_colons", "foo::bar::baz", "foo::bar", "foo"],
      "directives sort by descending target length",
    )
  }
  #[test]
  fn directive_ordering_by_span() -> Result<(), TestError> {
    // TODO(eliza): it would be nice to have a property-based test for this
    // instead.
    ensure_directive_target_order(
  ...
  #[test]
  fn directive_ordering_uses_lexicographic_when_equal() -> Result<(), TestError> {
    // TODO(eliza): it would be nice to have a property-based test for this
    // instead.
    ...
    let sorted = dirs
      .iter()
      .map(|directive| (directive.target.as_deref(), directive.in_span.as_deref()))
      .collect::<Vec<_>>();

    ensure(sorted == expected, "directives sort lexicographically after specificity")
      .map(drop)
      .map_err(TestError::from)
  }

  #[test]
  fn directive_ordering_by_field_num() -> Result<(), TestError> {
    // TODO(eliza): it would be nice to have a property-based test for this
    // instead.
    ensure_directive_target_order(
  ...
  #[test]
  fn parse_directives_ralith() -> Result<(), TestError> {
  ...
  #[test]
  fn parse_directives_ralith_uc() -> Result<(), TestError> {
    ensure_parses_ralith_case_insensitive("common=INFO,server=DEBUG")
  }

  #[test]
  fn parse_directives_ralith_mixed() -> Result<(), TestError> {
    ensure_parses_ralith_case_insensitive("common=iNfo,server=dEbUg")
  }

  #[test]
  fn parse_directives_valid() -> Result<(), TestError> {
  ...
  #[test]
  fn parse_level_directives() -> Result<(), TestError> {
    ensure_parses_level_directives(
      "crate1::mod1=error,crate1::mod2=warn,crate1::mod2::mod3=info,crate2=debug,crate3=trace,crate3::mod2::mod1=off",
    )
  }

  #[test]
  fn parse_uppercase_level_directives() -> Result<(), TestError> {
    ensure_parses_level_directives(
      "crate1::mod1=ERROR,crate1::mod2=WARN,crate1::mod2::mod3=INFO,crate2=DEBUG,crate3=TRACE,crate3::mod2::mod1=OFF",
    )
  }
  ...
  #[test]
  fn parse_numeric_level_directives() -> Result<(), TestError> {
    ensure_parses_level_directives("crate1::mod1=1,crate1::mod2=2,crate1::mod2::mod3=3,crate2=4,crate3=5,crate3::mod2::mod1=0")
  }

  #[test]
  fn parse_directives_invalid_crate() -> Result<(), TestError> {
    // test parse_directives with multiple = in specification
    let dirs = parse_directives("crate1::mod1=warn=info,crate2=debug");
    ensure_directives(&dirs, &[DirectiveExpectation {
      target:  Some("crate2"),
      level:   LevelFilter::DEBUG,
      in_span: None,
    }])
  ...
  #[test]
  fn parse_directives_invalid_level() -> Result<(), TestError> {
  ...
  #[test]
  fn parse_directives_string_level() -> Result<(), TestError> {
  ...
  #[test]
  fn parse_directives_empty_level() -> Result<(), TestError> {
  ...
  #[test]
  fn parse_directives_global() -> Result<(), TestError> {
  ...
  fn test_parse_bare_level(directive_to_test: &str, level_expected: LevelFilter) -> Result<(), TestError> {
  ...
  #[test]
  fn parse_directives_with_dash_in_target_name() -> Result<(), TestError> {
  ...
  #[test]
  fn parse_directives_with_dash_in_span_name() -> Result<(), TestError> {
  ...
  #[test]
  fn parse_directives_with_special_characters_in_span_name() -> Result<(), TestError> {
    let span_name = "!\"#$%&'()*+-./:;<=>?@^_`|~[}";
  ...
  #[test]
  fn parse_directives_with_invalid_span_chars() -> Result<(), TestError> {
    let invalid_span_name = "]{";

    let dirs = parse_directives(format!("target[{invalid_span_name}]=info"));
    ensure_eq(dirs.len(), 0_usize, "invalid span characters reject directive")
      .map(drop)
      .map_err(TestError::from)
  }
}
