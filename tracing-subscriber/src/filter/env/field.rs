...
impl Match {
  ...
  pub(crate) fn parse(source: &str, regex: bool) -> Result<Self, Box<dyn Error + Send + Sync>> {
...
impl SpanMatch {
  ...
  fn is_matched_slow(&self) -> bool {
    let matched = self.fields.values().all(|entry| entry.1.load(Acquire));
    if matched {
...
mod tests {
  ...
  enum TestError {
    ...
    #[error(transparent)]
    OptionMatch(#[from] Box<strict_test_support::OptionFailure<Match>>),
    /// Preserves the complete native failure and its inputs.
    #[error(transparent)]
    OptionMatchDebug(#[from] Box<strict_test_support::OptionFailure<MatchDebug>>),
    /// Preserves the complete native failure and its inputs.
    #[error(transparent)]
    OptionMatchPattern(#[from] Box<strict_test_support::OptionFailure<MatchPattern>>),
    /// Preserves the complete native failure and its inputs.
    #[error(transparent)]
    OptionValueMatch(#[from] Box<strict_test_support::OptionFailure<ValueMatch>>),
    /// Preserves the complete native failure and its inputs.
    #[error(transparent)]
    OptionField(#[from] Box<strict_test_support::OptionFailure<Field>>),
  }
  ...
  fn matcher_field(name: &str) -> Result<Field, TestError> {
    ensure_some(
  ...
  fn parse_field_matcher(source: &str, regex: bool) -> Result<Match, TestError> {
    ensure_some(Match::parse(source, regex).ok(), "field matcher source should parse")
      .map_err(Box::new)
      .map_err(TestError::from)
  }

  fn parse_regex_matcher(source: &str) -> Result<ValueMatch, TestError> {
    ensure_some(ValueMatch::parse_regex(source).ok(), "regex value matcher should parse")
      .map_err(Box::new)
      .map_err(TestError::from)
  }

  fn parse_exact_matcher(source: &str) -> Result<ValueMatch, TestError> {
    ensure_some(ValueMatch::parse_non_regex(source).ok(), "exact value matcher should parse")
      .map_err(Box::new)
      .map_err(TestError::from)
  }

  fn insert_matcher(field_matchers: &mut FieldMap<ValueMatch>, field: &Field, matcher: ValueMatch) -> Result<(), TestError> {
    ensure(
  ...
  #[test]
  fn match_parser_orders_value_matchers_after_name_only_matchers() -> Result<(), TestError> {
    let plain_matcher = parse_field_matcher("plain_field", true)?;
    let bool_matcher = parse_field_matcher("plain_field=true", true)?;

    ensure(!plain_matcher.has_value(), "plain field matcher has no value matcher").map(drop)?;
    ensure(bool_matcher.has_value(), "field value matcher reports its value matcher").map(drop)?;
    ensure(
  ...
  #[test]
  fn value_matchers_accept_matching_literals_and_reject_wrong_types() -> Result<(), TestError> {
    let bool_matcher = parse_regex_matcher("true")?;
    ensure(bool_matcher.matches_bool(true), "bool matcher accepts the matching bool").map(drop)?;
    ensure(!bool_matcher.matches_i64(1), "bool matcher rejects numeric fields").map(drop)?;

    let unsigned_matcher = parse_regex_matcher("7")?;
    ensure(unsigned_matcher.matches_u64(7), "unsigned matcher accepts matching u64").map(drop)?;
    ensure(
      unsigned_matcher.matches_i64(7),
      "unsigned matcher accepts non-negative signed values with the same magnitude",
    )
    .map(drop)?;
    ensure(!unsigned_matcher.matches_i64(-7), "unsigned matcher rejects negative signed values").map(drop)?;

    let signed_matcher = parse_regex_matcher("-7")?;
    ensure(signed_matcher.matches_i64(-7), "signed matcher accepts matching i64").map(drop)?;
    ensure(!signed_matcher.matches_u64(7), "signed matcher rejects unsigned fields").map(drop)?;

    let float_matcher = parse_regex_matcher("1.5")?;
    ensure(float_matcher.matches_f64(1.5), "float matcher accepts matching f64").map(drop)?;
    ensure(!float_matcher.matches_f64(2.5), "float matcher rejects another f64").map(drop)?;

    let nan_matcher = parse_regex_matcher("NaN")?;
    ensure(nan_matcher.matches_f64(f64::NAN), "NaN matcher accepts NaN").map(drop)?;
  ...
  #[test]
  fn regex_and_exact_debug_matchers_have_distinct_string_polarity() -> Result<(), TestError> {
  ...
  #[test]
  fn span_match_records_all_expected_fields_before_releasing_level() -> Result<(), TestError> {
    ...
    insert_matcher(&mut field_matchers, &name_field, parse_regex_matcher("alice.*")?)?;
    insert_matcher(
      &mut field_matchers,
      &debug_field,
    ...
    ensure(!span_match.is_matched(), "new span matcher should not be matched").map(drop)?;
    ensure(span_match.filter().is_none(), "unmatched span matcher should not release a level").map(drop)?;
    ...
    ensure(!span_match.is_matched(), "one wrong field should keep the span matcher unmatched").map(drop)?;

    visitor.record_bool(&enabled_field, true);
    ensure(span_match.is_matched(), "all matching fields should satisfy the span matcher").map(drop)?;
    ensure(
  ...
  #[test]
  fn value_match_ordering_and_equality_are_stable_by_type_then_value() -> Result<(), TestError> {
    ...
    ensure(bool_false < bool_true, "bool matchers order by bool value").map(drop)?;
    ensure(bool_true < f64_one, "bool matchers sort before f64 matchers").map(drop)?;
    ensure(f64_one < f64_two, "f64 matchers order by numeric value").map(drop)?;
    ensure(f64_two < ValueMatch::NaN, "finite f64 matchers sort before NaN").map(drop)?;
    ensure(ValueMatch::NaN < unsigned_one, "NaN matcher sorts before unsigned matchers").map(drop)?;
    ensure(unsigned_one < unsigned_two, "unsigned matchers order by numeric value").map(drop)?;
    ensure(unsigned_two < signed_negative, "unsigned matchers sort before signed matchers").map(drop)?;
    ensure(signed_negative < signed_positive, "signed matchers order by numeric value").map(drop)?;
    ensure(signed_positive < debug_alpha, "signed matchers sort before exact debug matchers").map(drop)?;
    ensure(debug_alpha < debug_beta, "exact debug matchers order by pattern").map(drop)?;
    ensure(debug_beta < pattern_alpha, "exact debug matchers sort before regex matchers").map(drop)?;
    ensure(pattern_alpha < pattern_beta, "regex matchers order by pattern").map(drop)?;
    ensure(ValueMatch::NaN == ValueMatch::NaN, "NaN matchers compare equal by matcher kind").map(drop)?;
    ensure(
  ...
  #[test]
  fn field_match_and_value_displays_preserve_user_directive_text() -> Result<(), TestError> {
    let exact = ensure_some(MatchDebug::new("alpha.*").ok(), "exact debug matcher should compile").map_err(Box::new)?;
    let pattern = ensure_some("alpha.*".parse::<MatchPattern>().ok(), "regex matcher should compile").map_err(Box::new)?;

    ensure(format!("{exact}") == "alpha.*", "exact debug display preserves the pattern").map(drop)?;
    ensure(exact.as_ref() == "alpha.*", "exact debug as_ref exposes the pattern").map(drop)?;
    ensure(format!("{pattern}") == "alpha.*", "regex display preserves the pattern").map(drop)?;
    ensure(pattern.as_ref() == "alpha.*", "regex as_ref exposes the pattern").map(drop)?;

    let exact_beta = ensure_some(MatchDebug::new("beta").ok(), "second exact matcher compiles").map_err(Box::new)?;
    let pattern_beta = ensure_some("beta.*".parse::<MatchPattern>().ok(), "second regex matcher compiles").map_err(Box::new)?;
    ensure(exact < exact_beta, "exact debug matchers order by pattern").map(drop)?;
    ensure(pattern < pattern_beta, "regex matchers order by pattern").map(drop)?;

    let nan_matcher = parse_regex_matcher("NaN")?;
    ensure(format!("{nan_matcher}") == "NaN", "NaN display round-trips through f64 display").map(drop)?;
  ...
  #[test]
  fn field_matcher_parsing_reports_invalid_regex_and_bad_names() -> Result<(), TestError> {
    ensure(
  ...
  #[test]
  fn span_match_visitors_ignore_unregistered_fields_and_wrong_value_types() -> Result<(), TestError> {
    let enabled_field = matcher_field("enabled")?;
    let count_field = matcher_field("count")?;
    ...
    visitor.record_bool(&enabled_field, true);
    ensure(!span_match.is_matched(), "one matching field is insufficient").map(drop)?;
  ...
  #[test]
  fn debug_struct_match() -> Result<(), TestError> {
    ...
    let matcher = ensure_some(MatchDebug::new(pattern).ok(), "debug pattern should compile").map_err(Box::new)?;
    ensure(matcher.debug_matches(&my_struct), "debug matcher accepts matching struct")
      .map(drop)
      .map_err(TestError::from)
  }

  #[test]
  fn debug_struct_not_match() -> Result<(), TestError> {
    let my_struct = MyStruct {
      answer:   42,
    ...
    let pattern = "MyStruct { answer: 42, question: \"life, the universe, and everything\" }";
      ...
      format!("{my_struct:?}") == "MyStruct { answer: 42, question: \"what shall we have for lunch?\" }",
      "`MyStruct`'s `Debug` impl outputs the expected non-matching string",
    )
    .map(drop)?;

    let matcher = ensure_some(MatchDebug::new(pattern).ok(), "debug pattern should compile").map_err(Box::new)?;
    ensure(!matcher.debug_matches(&my_struct), "debug matcher rejects non-matching struct")
      .map(drop)
      .map_err(TestError::from)
  }
}
