...
//! # = &reload_handle;
//! #
//! tracing_subscriber::registry().with(filtered_layer).try_init()?;
//! info!("This will be ignored");
  ...
  S: Subscriber,
{
  ...
  fn downcast_ref_by_id(&self, id: TypeId) -> Option<&dyn Any> {
    ...
    if id == TypeId::of::<layer::NoneLayerMarker>() {
      let marker_present = try_lock!(self.inner.read(), else return None).downcast_ref_by_id(id).is_some();
      if marker_present {
...
mod tests {
  ...
  enum TestError {
    ...
    /// Preserves the complete native failure and its inputs.
    #[error(transparent)]
    ResultReloadError(#[from] strict_test_support::ResultFailure<ReloadError>),
    /// Preserves the complete native failure and its inputs.
    #[error(transparent)]
  ...
  impl layer::Layer<registry::Registry> for RecordingLayer {
    ...
    fn on_new_span(&self, _attrs: &Attributes<'_>, _id: Id, _ctx: layer::Context<'_, registry::Registry>) -> SubscriberResult<()> {
      self.record("on_new_span");
      Ok(())
    }

    fn on_record(&self, _span: Id, _values: &Record<'_>, _ctx: layer::Context<'_, registry::Registry>) -> SubscriberResult<()> {
      self.record("on_record");
      Ok(())
    }

    fn on_follows_from(&self, _span: Id, _follows: Id, _ctx: layer::Context<'_, registry::Registry>) -> SubscriberResult<()> {
    ...
    fn on_id_change(&self, _old: Id, _new: Id, _ctx: layer::Context<'_, registry::Registry>) -> SubscriberResult<()> {
  ...
  fn with_reload_fixtures<R>(
    f: impl FnOnce(&Event<'_>, &Attributes<'_>, &Record<'_>, Id, Id) -> Result<R, TestError>,
  ) -> Result<R, TestError> {
  ...
  #[test]
  fn handle_reads_modifies_reloads_and_reports_dropped_state() -> Result<(), TestError> {
    let (layer, handle): (NumberReloadLayer, NumberReloadHandle) = Layer::new(1);
    ensure(handle.clone_current() == Some(1), "clone_current returns the initial value").map(drop)?;
    let current = ensure_ok(handle.with_current(|inner| *inner), "with_current reads")?;
    ensure_eq(current, 1, "with_current borrows the current value").map(drop)?;
    ensure_ok(
      handle.modify(|inner| {
        *inner = 2;
      }),
    ...
    ensure(handle.clone_current() == Some(2), "modify result is visible to clone_current").map(drop)?;
    let cloned = handle.clone();
    ensure_ok(cloned.reload(3_usize), "cloned handles can reload")?;
    ensure(handle.clone_current() == Some(3), "reload replaces the current value").map(drop)?;

    let layer_debug = format!("{layer:?}");
    ensure_contains(layer_debug, String::from("Layer"), "reload layer debug names the wrapper").map(drop)?;
    let handle_debug = format!("{handle:?}");
    ensure_contains(handle_debug, String::from("Handle"), "reload handle debug names the wrapper").map(drop)?;
    ...
    ensure(error.is_dropped(), "dropped reload errors report dropped state").map(drop)?;
    ensure(!error.is_poisoned(), "dropped reload errors are not poisoned errors").map(drop)?;
    let display = format!("{error}");
    ensure_contains(
      display,
      String::from("subscriber no longer exists"),
      "dropped error display is stable",
    )
    .map(drop)?;
    let debug = format!("{error:?}");
    ensure_contains(
      debug,
      String::from("SubscriberGone"),
      "dropped error debug preserves the error kind",
    )
    .map(drop)?;
  ...
  #[test]
  fn reload_layer_forwards_registration_and_metadata_hooks() -> Result<(), TestError> {
    with_reload_fixtures(|event, _attrs, _record, _first_id, _second_id| {
      ...
      ensure_ok(
        reload_layer.on_register_dispatch(&dispatch),
        "reload layer forwards dispatch registration",
      )?;
      reload_layer.on_layer(&mut subscriber);
      ensure(
        ensure_ok(
          reload_layer.register_callsite(&RELOAD_META),
          "reload layer forwards callsite registration",
        )?
        .is_always(),
      ...
      let expected = vec![
        "on_register_dispatch", "on_layer", "register_callsite", "enabled", "event_enabled", "max_level_hint",
      ];
  ...
  #[test]
  fn reload_layer_forwards_span_and_event_lifecycle_hooks() -> Result<(), TestError> {
    with_reload_fixtures(|event, attrs, record, first_id, second_id| {
      ...
      ensure_ok(reload_layer.on_event(event, context.clone()), "reload layer forwards events")?;
      ...
      let expected = vec![
        "on_new_span", "on_record", "on_follows_from", "on_event", "on_enter", "on_exit", "on_close", "on_id_change",
      ];
  ...
  #[test]
  fn reload_layer_downcast_exposes_only_disabled_option_layer_marker() -> Result<(), TestError> {
    let (none_layer, _none_handle): (OptionalIdentityReloadLayer, OptionalIdentityReloadHandle) = Layer::new(None);
    ensure(
      none_layer.downcast_ref_by_id(TypeId::of::<layer::NoneLayerMarker>()).is_some(),
      "reload layer exposes the disabled optional layer marker",
    )
    .map(drop)?;
    ensure(
      none_layer.downcast_ref_by_id(TypeId::of::<Option<layer::Identity>>()).is_none(),
    ...
    let (some_layer, _some_handle): (OptionalIdentityReloadLayer, OptionalIdentityReloadHandle) =
      ...
      some_layer.downcast_ref_by_id(TypeId::of::<layer::NoneLayerMarker>()).is_none(),
  ...
  #[test]
  fn reload_layer_forwards_filter_hooks() -> Result<(), TestError> {
    with_reload_fixtures(|_event, attrs, record, first_id, _second_id| {
      ...
      .map(drop)?;
        ...
        !ensure_ok(filter.enabled(&RELOAD_META, &context), "reload filter forwards enabled")?,
      ...
      let expected = vec![
        "callsite_enabled", "enabled", "on_new_span", "on_record", "on_enter", "on_exit", "on_close", "max_level_hint",
      ];
