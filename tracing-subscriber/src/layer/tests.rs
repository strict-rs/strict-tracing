...
use tracing_core::subscriber::{NoSubscriber, SubscriberResult};
...
fn downcasts_to_subscriber() -> Result<(), TestFailure> {
    ...
    let downcast_subscriber = ensure_some(
        ...
        downcast_subscriber.0 == "subscriber",
        "downcast subscriber preserves inner value",
    )
}
...
mod registry_tests {
        ...
        subscriber::with_default(
            ...
            || {
                tracing::info!("no span");
                    ...
                    last_event_span.lock().is_none(),
                    "event outside a span has no event span",
                )?;

                let parent = tracing::info_span!("explicit");
                tracing::info!(parent: &parent, "explicit span");
                ensure(
                    *last_event_span.lock() == Some("explicit"),
                    "explicit parent is event span",
                )?;

                let _guard = tracing::info_span!("contextual").entered();
                tracing::info!("contextual span");
                ensure(
                    *last_event_span.lock() == Some("contextual"),
                    "entered span is event span",
                )
            },
        )
    }
    ...
    mod max_level_hints {
        ...
        fn ensure_max_level_hint(
            actual: Option<LevelFilter>,
