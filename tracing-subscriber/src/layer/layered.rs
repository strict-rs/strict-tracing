  ...
  I: Subscriber,
{
  ...
  fn register_callsite(&self, metadata: &'static Metadata<'static>) -> SubscriberResult<Interest> {
    self.pick_interest(self.layer.register_callsite(metadata)?, || self.inner.register_callsite(metadata))
  }
  ...
  fn max_level_hint(&self) -> Option<LevelFilter> {
    self.pick_level_hint(
      ...
      self.inner.downcast_ref_by_id(TypeId::of::<super::NoneLayerMarker>()).is_some(),
    )
  }
  ...
  fn try_close(&self, id: span::Id) -> SubscriberResult<bool> {
    ...
    #[cfg(all(feature = "registry", feature = "std"))]
    let mut close_handle = subscriber.downcast_ref::<Registry>().map(|registry| registry.start_close(id));
  ...
  fn downcast_ref_by_id(&self, id: TypeId) -> Option<&dyn Any> {
    ...
    self.layer.downcast_ref_by_id(id).or_else(|| self.inner.downcast_ref_by_id(id))
  }
}
  ...
  S: Subscriber,
{
  ...
  fn downcast_ref_by_id(&self, id: TypeId) -> Option<&dyn Any> {
    match id {
      ...
      _ => self.layer.downcast_ref_by_id(id).or_else(|| self.inner.downcast_ref_by_id(id)),
    }
  }
}
  ...
  S: Subscriber,
{
  ...
  fn pick_interest(&self, outer: Interest, inner_interest: impl FnOnce() -> SubscriberResult<Interest>) -> SubscriberResult<Interest> {
  ...
  fn pick_level_hint(&self, outer_hint: Option<LevelFilter>, inner_hint: Option<LevelFilter>, inner_is_none: bool) -> Option<LevelFilter> {
    ...
    #[cfg(all(feature = "registry", feature = "std"))]
    {
      if alt {
        let _debug = debug
          ...
          .field("inner_has_layer_filter", &self.filters.inner_has_layer_filter());
      }
    }

    debug.field("layer", &self.layer).field("inner", &self.inner).finish()
  }
}
