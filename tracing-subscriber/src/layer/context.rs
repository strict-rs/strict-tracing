  ...
  S: Subscriber,
{
  ...
  /// # Notes
  ///
  /// - The subscriber is free to expect that the event's callsite has been [registered][register],
  ///   and may panic or fail to observe the event if this is not the case. The `tracing` crate's
  ///   macros ensure that all events are registered, but if the event is constructed through other
  ///   means, the user is responsible for ensuring that [`register_callsite`][register] has been
  ///   called prior to calling this method.
  /// - This does _not_ call [`enabled`] on the inner subscriber. If the caller wishes to apply the
  ///   wrapped subscriber's filter before choosing whether to record the event, it may first call
  ///   [`Context::enabled`] to check whether the event would be enabled. This allows `Layer`s to
  ...
  where
    S: for<'lookup> LookupSpan<'lookup>,
  {
    self.subscriber.as_ref().and_then(|subscriber| subscriber.span(id)).is_some()
  }
  ...
  #[inline(never)]
  #[cfg(all(feature = "registry", feature = "std"))]
  fn lookup_current_filtered<'lookup>(&self, subscriber: &'lookup S) -> Option<SpanRef<'lookup, S>>
  where
    S: LookupSpan<'lookup>,
  {
  ...
  where
    S: for<'lookup> LookupSpan<'lookup>,
  {
    self.is_enabled_inner(span, filter)?.then(|| self.with_filter(filter))
  }
