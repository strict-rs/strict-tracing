...
//!
//! * [`Full`]: The default formatter. This emits human-readable, single-line logs for each event
//!   that occurs, with the current span context displayed before the formatted representation of
//!   the event. See [here](Full#example-output) for sample output.
//!
//! * [`Compact`]: A variant of the default formatter, optimized for short line lengths. Fields from
//!   the current span context are appended to the fields of the formatted event, and span names are
//!   not shown; the verbosity level is abbreviated to a single character. See
//!   [here](Compact#example-output) for sample output.
//!
//! * [`Pretty`]: Emits excessively pretty, multi-line logs, optimized for human readability. This
//!   is primarily intended to be used in local development and debugging, or for command-line
//!   applications, where automated analysis and compact storage of logs is less of a priority than
//!   readability and visual appeal. See [here](Pretty#example-output) for sample output.
//!
//! * [`Json`]: Outputs newline-delimited JSON logs. This is intended for production use with
//!   systems where structured logs are consumed as JSON by analysis and viewing tools. The JSON
//!   output is not optimized for human readability. See [here](Json#example-output) for sample
//!   output.
use std::any;
use std::error::Error;
use std::fmt::Debug;
...
use crate::field::MakeOutput;
use crate::field::MakeVisitor;
use crate::field::RecordFields;
use crate::field::VisitFmt;
use crate::field::VisitOutput;
use crate::fmt::fmt_layer::FmtContext;
use crate::fmt::fmt_layer::FormattedFields;
...
///
/// * A [`FmtContext`]. This is an extension of the [`layer::Context`] type, which can be used for
///   accessing stored information such as the current span context an event occurred in.
///
...
///
/// * A [`Writer`] to which the formatted representation of the event is written. This type
///   implements the [`std::fmt::Write`] trait, and therefore can be used with the [`std::write!`]
///   and [`std::writeln!`] macros, as well as calling [`std::fmt::Write`] methods directly.
///
...
/// let _subscriber = tracing_subscriber::fmt().event_format(MyFormatter).try_init()?;
///
...
impl<S, N> FormatEvent<S, N> for fn(ctx: &FmtContext<'_, S, N>, Writer<'_>, &Event<'_>) -> fmt::Result
where
  S: Subscriber + for<'a> LookupSpan<'a>,
  N: for<'a> FormatFields<'a> + 'static,
{
...
/// tracing_subscriber::fmt().event_format(format).try_init()?;
/// # Ok(()) }
/// ```
#[must_use]
#[allow(
...
/// # Example Output
///
/// <pre><font color="#4E9A06"><b>:;</b></font> <font color="#4E9A06">cargo</font> run --example
/// fmt-compact <font color="#4E9A06"><b>    Finished</b></font> dev [unoptimized + debuginfo]
/// target(s) in 0.08s <font color="#4E9A06"><b>     Running</b></font>
/// `target/debug/examples/fmt-compact` <font color="#AAAAAA">2022-02-17T19:51:05.809287Z
/// </font><font color="#4E9A06"> INFO</font> <b>fmt_compact</b><font color="#AAAAAA">: preparing to
/// shave yaks </font><i>number_of_yaks</i><font color="#AAAAAA">=3</font> <font color="#AAAAAA"
/// >2022-02-17T19:51:05.809367Z </font><font color="#4E9A06"> INFO</font> <b>shaving_yaks</b>:
/// <b>fmt_compact::yak_shave</b><font color="#AAAAAA">: shaving yaks </font><font
/// color="#AAAAAA"><i>yaks</i></font><font color="#AAAAAA">=3</font> <font color="#AAAAAA"
/// >2022-02-17T19:51:05.809414Z </font><font color="#75507B">TRACE</font>
/// <b>shaving_yaks</b>:<b>shave</b>: <b>fmt_compact::yak_shave</b><font color="#AAAAAA">: hello!
/// I&apos;m gonna shave a yak </font><i>excitement</i><font color="#AAAAAA">=&quot;yay!&quot;
/// </font><font color="#AAAAAA"><i>yaks</i></font><font color="#AAAAAA">=3 </font><font
/// color="#AAAAAA"><i>yak</i></font><font color="#AAAAAA">=1</font> <font color="#AAAAAA"
/// >2022-02-17T19:51:05.809443Z </font><font color="#75507B">TRACE</font>
/// <b>shaving_yaks</b>:<b>shave</b>: <b>fmt_compact::yak_shave</b><font color="#AAAAAA">: yak
/// shaved successfully </font><font color="#AAAAAA"><i>yaks</i></font><font color="#AAAAAA">=3
/// </font><font color="#AAAAAA"><i>yak</i></font><font color="#AAAAAA">=1</font> <font color="#
/// AAAAAA">2022-02-17T19:51:05.809477Z </font><font color="#3465A4">DEBUG</font>
/// <b>shaving_yaks</b>: <b>yak_events</b><font color="#AAAAAA">: </font><i>yak</i><font
/// color="#AAAAAA">=1 </font><i>shaved</i><font color="#AAAAAA">=true </font><font
/// color="#AAAAAA"><i>yaks</i></font><font color="#AAAAAA">=3</font> <font color="#AAAAAA"
/// >2022-02-17T19:51:05.809500Z </font><font color="#75507B">TRACE</font> <b>shaving_yaks</b>:
/// <b>fmt_compact::yak_shave</b><font color="#AAAAAA">: </font><i>yaks_shaved</i><font
/// color="#AAAAAA">=1 </font><font color="#AAAAAA"><i>yaks</i></font><font
/// color="#AAAAAA">=3</font> <font color="#AAAAAA">2022-02-17T19:51:05.809531Z </font><font
/// color="#75507B">TRACE</font> <b>shaving_yaks</b>:<b>shave</b>:
/// <b>fmt_compact::yak_shave</b><font color="#AAAAAA">: hello! I&apos;m gonna shave a yak
/// </font><i>excitement</i><font color="#AAAAAA">=&quot;yay!&quot; </font><font
/// color="#AAAAAA"><i>yaks</i></font><font color="#AAAAAA">=3 </font><font
/// color="#AAAAAA"><i>yak</i></font><font color="#AAAAAA">=2</font> <font color="#AAAAAA"
/// >2022-02-17T19:51:05.809554Z </font><font color="#75507B">TRACE</font>
/// <b>shaving_yaks</b>:<b>shave</b>: <b>fmt_compact::yak_shave</b><font color="#AAAAAA">: yak
/// shaved successfully </font><font color="#AAAAAA"><i>yaks</i></font><font color="#AAAAAA">=3
/// </font><font color="#AAAAAA"><i>yak</i></font><font color="#AAAAAA">=2</font> <font color="#
/// AAAAAA">2022-02-17T19:51:05.809581Z </font><font color="#3465A4">DEBUG</font>
/// <b>shaving_yaks</b>: <b>yak_events</b><font color="#AAAAAA">: </font><i>yak</i><font
/// color="#AAAAAA">=2 </font><i>shaved</i><font color="#AAAAAA">=true </font><font
/// color="#AAAAAA"><i>yaks</i></font><font color="#AAAAAA">=3</font> <font color="#AAAAAA"
/// >2022-02-17T19:51:05.809606Z </font><font color="#75507B">TRACE</font> <b>shaving_yaks</b>:
/// <b>fmt_compact::yak_shave</b><font color="#AAAAAA">: </font><i>yaks_shaved</i><font
/// color="#AAAAAA">=2 </font><font color="#AAAAAA"><i>yaks</i></font><font
/// color="#AAAAAA">=3</font> <font color="#AAAAAA">2022-02-17T19:51:05.809635Z </font><font
/// color="#75507B">TRACE</font> <b>shaving_yaks</b>:<b>shave</b>:
/// <b>fmt_compact::yak_shave</b><font color="#AAAAAA">: hello! I&apos;m gonna shave a yak
/// </font><i>excitement</i><font color="#AAAAAA">=&quot;yay!&quot; </font><font
/// color="#AAAAAA"><i>yaks</i></font><font color="#AAAAAA">=3 </font><font
/// color="#AAAAAA"><i>yak</i></font><font color="#AAAAAA">=3</font> <font color="#AAAAAA"
/// >2022-02-17T19:51:05.809664Z </font><font color="#C4A000"> WARN</font>
/// <b>shaving_yaks</b>:<b>shave</b>: <b>fmt_compact::yak_shave</b><font color="#AAAAAA">: could not
/// locate yak </font><font color="#AAAAAA"><i>yaks</i></font><font color="#AAAAAA">=3 </font><font
/// color="#AAAAAA"><i>yak</i></font><font color="#AAAAAA">=3</font> <font color="#AAAAAA"
/// >2022-02-17T19:51:05.809693Z </font><font color="#3465A4">DEBUG</font> <b>shaving_yaks</b>:
/// <b>yak_events</b><font color="#AAAAAA">: </font><i>yak</i><font color="#AAAAAA">=3
/// </font><i>shaved</i><font color="#AAAAAA">=false </font><font
/// color="#AAAAAA"><i>yaks</i></font><font color="#AAAAAA">=3</font> <font color="#AAAAAA"
/// >2022-02-17T19:51:05.809717Z </font><font color="#CC0000">ERROR</font> <b>shaving_yaks</b>:
/// <b>fmt_compact::yak_shave</b><font color="#AAAAAA">: failed to shave yak </font><i>yak</i><font
/// color="#AAAAAA">=3 </font><i>error</i><font color="#AAAAAA">=missing yak
/// </font><i>error.sources</i><font color="#AAAAAA">=[out of space, out of cash] </font><font
/// color="#AAAAAA"><i>yaks</i></font><font color="#AAAAAA">=3</font> <font color="#AAAAAA"
/// >2022-02-17T19:51:05.809743Z </font><font color="#75507B">TRACE</font> <b>shaving_yaks</b>:
/// <b>fmt_compact::yak_shave</b><font color="#AAAAAA">: </font><i>yaks_shaved</i><font
/// color="#AAAAAA">=2 </font><font color="#AAAAAA"><i>yaks</i></font><font
/// color="#AAAAAA">=3</font> <font color="#AAAAAA">2022-02-17T19:51:05.809768Z </font><font
/// color="#4E9A06"> INFO</font> <b>fmt_compact</b><font color="#AAAAAA">: yak shaving completed
/// </font><i>all_yaks_shaved</i><font color="#AAAAAA">=false</font>
///
/// </pre>
...
/// <font color="#4E9A06"><b>    Finished</b></font> dev [unoptimized + debuginfo] target(s) in
/// 0.08s <font color="#4E9A06"><b>     Running</b></font> `target/debug/examples/fmt`
/// <font color="#AAAAAA">2022-02-15T18:40:14.289898Z </font><font color="#4E9A06"> INFO</font> fmt:
/// preparing to shave yaks <i>number_of_yaks</i><font color="#AAAAAA">=3</font> <font color="#
/// AAAAAA">2022-02-15T18:40:14.289974Z </font><font color="#4E9A06"> INFO</font>
/// <b>shaving_yaks{</b><i>yaks</i><font color="#AAAAAA">=3</font><b>}</b><font color="#AAAAAA">:
/// fmt::yak_shave: shaving yaks</font> <font color="#AAAAAA">2022-02-15T18:40:14.290011Z
/// </font><font color="#75507B">TRACE</font> <b>shaving_yaks{</b><i>yaks</i><font
/// color="#AAAAAA">=3</font><b>}</b><font color="#AAAAAA">:</font><b>shave{</b><i>yak</i><font
/// color="#AAAAAA">=1</font><b>}</b><font color="#AAAAAA">: fmt::yak_shave: hello! I&apos;m gonna
/// shave a yak </font><i>excitement</i><font color="#AAAAAA">=&quot;yay!&quot;</font> <font color="
/// #AAAAAA">2022-02-15T18:40:14.290038Z </font><font color="#75507B">TRACE</font>
/// <b>shaving_yaks{</b><i>yaks</i><font color="#AAAAAA">=3</font><b>}</b><font
/// color="#AAAAAA">:</font><b>shave{</b><i>yak</i><font color="#AAAAAA">=1</font><b>}</b><font
/// color="#AAAAAA">: fmt::yak_shave: yak shaved successfully</font> <font color="#AAAAAA"
/// >2022-02-15T18:40:14.290070Z </font><font color="#3465A4">DEBUG</font>
/// <b>shaving_yaks{</b><i>yaks</i><font color="#AAAAAA">=3</font><b>}</b><font color="#AAAAAA">:
/// yak_events: </font><i>yak</i><font color="#AAAAAA">=1 </font><i>shaved</i><font
/// color="#AAAAAA">=true</font> <font color="#AAAAAA">2022-02-15T18:40:14.290089Z </font><font
/// color="#75507B">TRACE</font> <b>shaving_yaks{</b><i>yaks</i><font
/// color="#AAAAAA">=3</font><b>}</b><font color="#AAAAAA">: fmt::yak_shave:
/// </font><i>yaks_shaved</i><font color="#AAAAAA">=1</font> <font color="#AAAAAA">2022-02-15T18:40:
/// 14.290114Z </font><font color="#75507B">TRACE</font> <b>shaving_yaks{</b><i>yaks</i><font
/// color="#AAAAAA">=3</font><b>}</b><font color="#AAAAAA">:</font><b>shave{</b><i>yak</i><font
/// color="#AAAAAA">=2</font><b>}</b><font color="#AAAAAA">: fmt::yak_shave: hello! I&apos;m gonna
/// shave a yak </font><i>excitement</i><font color="#AAAAAA">=&quot;yay!&quot;</font> <font color="
/// #AAAAAA">2022-02-15T18:40:14.290134Z </font><font color="#75507B">TRACE</font>
/// <b>shaving_yaks{</b><i>yaks</i><font color="#AAAAAA">=3</font><b>}</b><font
/// color="#AAAAAA">:</font><b>shave{</b><i>yak</i><font color="#AAAAAA">=2</font><b>}</b><font
/// color="#AAAAAA">: fmt::yak_shave: yak shaved successfully</font> <font color="#AAAAAA"
/// >2022-02-15T18:40:14.290157Z </font><font color="#3465A4">DEBUG</font>
/// <b>shaving_yaks{</b><i>yaks</i><font color="#AAAAAA">=3</font><b>}</b><font color="#AAAAAA">:
/// yak_events: </font><i>yak</i><font color="#AAAAAA">=2 </font><i>shaved</i><font
/// color="#AAAAAA">=true</font> <font color="#AAAAAA">2022-02-15T18:40:14.290174Z </font><font
/// color="#75507B">TRACE</font> <b>shaving_yaks{</b><i>yaks</i><font
/// color="#AAAAAA">=3</font><b>}</b><font color="#AAAAAA">: fmt::yak_shave:
/// </font><i>yaks_shaved</i><font color="#AAAAAA">=2</font> <font color="#AAAAAA">2022-02-15T18:40:
/// 14.290198Z </font><font color="#75507B">TRACE</font> <b>shaving_yaks{</b><i>yaks</i><font
/// color="#AAAAAA">=3</font><b>}</b><font color="#AAAAAA">:</font><b>shave{</b><i>yak</i><font
/// color="#AAAAAA">=3</font><b>}</b><font color="#AAAAAA">: fmt::yak_shave: hello! I&apos;m gonna
/// shave a yak </font><i>excitement</i><font color="#AAAAAA">=&quot;yay!&quot;</font> <font color="
/// #AAAAAA">2022-02-15T18:40:14.290222Z </font><font color="#C4A000"> WARN</font>
/// <b>shaving_yaks{</b><i>yaks</i><font color="#AAAAAA">=3</font><b>}</b><font
/// color="#AAAAAA">:</font><b>shave{</b><i>yak</i><font color="#AAAAAA">=3</font><b>}</b><font
/// color="#AAAAAA">: fmt::yak_shave: could not locate yak</font> <font color="#AAAAAA"
/// >2022-02-15T18:40:14.290247Z </font><font color="#3465A4">DEBUG</font>
/// <b>shaving_yaks{</b><i>yaks</i><font color="#AAAAAA">=3</font><b>}</b><font color="#AAAAAA">:
/// yak_events: </font><i>yak</i><font color="#AAAAAA">=3 </font><i>shaved</i><font
/// color="#AAAAAA">=false</font> <font color="#AAAAAA">2022-02-15T18:40:14.290268Z </font><font
/// color="#CC0000">ERROR</font> <b>shaving_yaks{</b><i>yaks</i><font
/// color="#AAAAAA">=3</font><b>}</b><font color="#AAAAAA">: fmt::yak_shave: failed to shave yak
/// </font><i>yak</i><font color="#AAAAAA">=3 </font><i>error</i><font color="#AAAAAA">=missing yak
/// </font><i>error.sources</i><font color="#AAAAAA">=[out of space, out of cash]</font>
/// <font color="#AAAAAA">2022-02-15T18:40:14.290287Z </font><font color="#75507B">TRACE</font>
/// <b>shaving_yaks{</b><i>yaks</i><font color="#AAAAAA">=3</font><b>}</b><font color="#AAAAAA">:
/// fmt::yak_shave: </font><i>yaks_shaved</i><font color="#AAAAAA">=2</font> <font color="#AAAAAA"
/// >2022-02-15T18:40:14.290309Z </font><font color="#4E9A06"> INFO</font> fmt: yak shaving
/// completed. <i>all_yaks_shaved</i><font color="#AAAAAA">=false</font> </pre>
#[derive(Default, Debug, Copy, Clone, Eq, PartialEq)]
pub struct Full;
    ...
    T: FormatTime,
  {
    ...
    #[cfg(feature = "ansi")]
    {
      if writer.has_ansi_escapes() {
        ...
        // formatting errors.
        self.timer.format_time(writer).or_else(|_| writer.write_str("<unknown time>"))?;
...
/// Returns a level formatter configured for the current writer.
#[cfg(feature = "ansi")]
const fn fmt_level<'a>(level: &'a Level, writer: &Writer<'_>) -> FmtLevel<'a> {
  FmtLevel {
    level,
...
/// Returns a level formatter configured for the current writer.
#[cfg(not(feature = "ansi"))]
const fn fmt_level<'a>(level: &'a Level, _writer: &Writer<'_>) -> FmtLevel<'a> {
  FmtLevel {
    level,
  }
}

/// Returns dimmed styling configured for the current writer.
#[cfg(feature = "ansi")]
fn dimmed_style(writer: &Writer<'_>) -> Style {
  writer.dimmed()
}

/// Returns dimmed styling configured for the current writer.
#[cfg(not(feature = "ansi"))]
const fn dimmed_style(_writer: &Writer<'_>) -> Style {
  Style::new()
}

/// Returns italic styling configured for the current writer.
#[cfg(feature = "ansi")]
fn italic_style(writer: &Writer<'_>) -> Style {
  writer.italic()
}

/// Returns italic styling configured for the current writer.
#[cfg(not(feature = "ansi"))]
const fn italic_style(_writer: &Writer<'_>) -> Style {
  Style::new()
}
  ...
  T: FormatTime,
{
  fn format_event(&self, ctx: &FmtContext<'_, S, N>, mut writer: Writer<'_>, event: &Event<'_>) -> fmt::Result {
    ...
    if self.display.level() {
      let fmt_level = fmt_level(meta.level(), &writer);
      write!(writer, "{fmt_level} ")?;
    }

    write_thread_context(self.display, &mut writer)?;

    let dimmed = dimmed_style(&writer);
    ...
    if let Some(scope) = ctx.event_scope() {
      ...
      for span in scope.root_to_leaf() {
        ...
        match ext.get::<FormattedFields<N>>() {
          Some(fields) if !fields.is_empty() => write!(writer, "{}{}{}", bold.paint("{"), fields, bold.paint("}")),
          Some(_) | None => Ok(()),
        }?;
        drop(ext);
    ...
    let maybe_line_number = if self.display.line_number() { meta.line() } else { None };
    ...
    if let Some(event_line_number) = maybe_line_number {
      write!(writer, "{}{}:{} ", dimmed.prefix(), event_line_number, dimmed.suffix())?;
    }
  ...
  T: FormatTime,
{
  fn format_event(&self, ctx: &FmtContext<'_, S, N>, mut writer: Writer<'_>, event: &Event<'_>) -> fmt::Result {
    ...
    {
      write!(writer, "{}{}{}{}", dimmed.prefix(), line_number, dimmed.suffix(), dimmed.paint(":"))?;
      needs_space = true;
    }
    ...
    for span in ctx.event_scope().into_iter().flat_map(Scope::root_to_leaf) {
...
impl Visit for DefaultVisitor<'_> {
  ...
  fn record_error(&mut self, field: &Field, field_value: &(dyn Error + 'static)) {
    ...
    if let Some(source) = field_value.source() {
      let italic = italic_style(&self.writer);
      let dimmed = dimmed_style(&self.writer);
      self.record_debug(
        field,
        &format_args!(
          "{} {}{}{}{}",
      ...
      self.record_debug(field, &EscapeGuard::new(format_args!("{field_value}"), sanitize));
    }
  }
...
impl Display for ErrorSourceList<'_> {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    while let Some(curr_err) = curr {
      let _list = list.entry(&EscapeGuard::new(format_args!("{curr_err}"), self.ansi_sanitization));
  ...
  N: for<'writer> FormatFields<'writer> + 'static,
{
  ...
  const fn new(ctx: &'a FmtContext<'_, S, N>, span: Option<&'a span::Id>, ansi: bool) -> Self {
    Self {
      ctx,
      span,
      ansi,
  ...
  N: for<'writer> FormatFields<'writer> + 'static,
{
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    let scope = current_span.into_iter().flat_map(|span_ref| span_ref.scope().root_to_leaf());
  ...
  F: Fn(&mut Writer<'a>, &Field, &dyn Debug) -> fmt::Result,
{
  fn record_debug(&mut self, field: &Field, field_value: &dyn Debug) {
    if self.result.is_ok() {
      self.result = (self.f)(&mut self.writer, field, field_value);
    }
  }
}
...
impl FmtSpan {
  ...
  pub const FULL: Self = Self(Self::NEW.0 | Self::ENTER.0 | Self::EXIT.0 | Self::CLOSE.0);
...
impl Display for TimingDisplay {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    for (unit, nanos_per_unit) in [("ns", 1_u128), ("\u{b5}s", 1_000), ("ms", 1_000_000), ("s", 1_000_000_000)] {
      if nanos < nanos_per_unit.checked_mul(10).ok_or(fmt::Error)? {
        return write_fixed_duration(f, nanos, nanos_per_unit, 2, unit);
      }
      if nanos < nanos_per_unit.checked_mul(100).ok_or(fmt::Error)? {
        return write_fixed_duration(f, nanos, nanos_per_unit, 1, unit);
      }
      if nanos < nanos_per_unit.checked_mul(1_000).ok_or(fmt::Error)? {
...
fn write_fixed_duration(f: &mut fmt::Formatter<'_>, nanos: u128, nanos_per_unit: u128, precision: u32, unit: &str) -> fmt::Result {
  ...
  let scaled = nanos
    .checked_mul(scale)
    .and_then(|scaled_nanos| scaled_nanos.checked_add(half_unit))
    .and_then(|scaled_nanos| scaled_nanos.checked_div(nanos_per_unit))
    .ok_or(fmt::Error)?;
...
pub(super) mod test {
  ...
  use crate::fmt::SubscriberBuilder;
  ...
  enum TestError {
    ...
    /// Retains the native regex failure.
    #[error(transparent)]
    Regex(#[from] strict_test_support::ResultFailure<regex::Error>),
  }
  ...
  type TestEventFormatter = fn(&FmtContext<'_, crate::Registry, DefaultFields>, Writer<'_>, &Event<'_>) -> fmt::Result;

  struct FailingTime;
  impl FormatTime for FailingTime {
    fn format_time(&self, _writer: &mut Writer<'_>) -> FmtResult {
      Err(fmt::Error)
    }
  }

  #[test]
  fn disable_everything() -> Result<(), TestError> {
  ...
  #[test]
  fn writer_forwards_fmt_writes_and_reports_configuration() -> Result<(), TestError> {
    let mut output = String::new();
    let writer_result = {
      let mut writer = Writer::new(&mut output).with_ansi_sanitization(false);

      ensure(!writer.has_ansi_escapes(), "new writers do not enable ANSI escapes by default").map(drop)?;
      ensure(!writer.sanitizes_ansi_escapes(), "writer exposes disabled ANSI sanitization").map(drop)?;
      ensure_ok(write!(writer.by_ref(), "alpha"), "by-ref writer forwards formatted writes")?;
      ensure_ok(writer.write_char(':'), "writer forwards character writes")?;
      ensure_ok(writer.write_fmt(format_args!("{}", "beta")), "writer forwards fmt arguments")?;

      let debug = format!("{writer:?}");
      ensure_contains(debug.clone(), String::from("Writer"), "writer debug names the adapter").map(drop)?;
      ensure_contains(debug.clone(), String::from("is_ansi: false"), "writer debug reports ANSI support").map(drop)?;
      ensure_contains(
        debug,
        String::from("ansi_sanitization: false"),
        "writer debug reports sanitization state",
      )
      .map(drop)?;
      Ok::<(), TestError>(())
    };
    ...
    ensure_eq(
      ...
      "writer forwards all writes to the inner buffer",
    )
    .map(drop)
  ...
  #[test]
  fn format_display_flags_toggle_each_output_axis() -> Result<(), TestError> {
    let defaults = FormatDisplay::default();
    ensure(defaults.timestamp(), "default display includes timestamps").map(drop)?;
    ensure(defaults.target(), "default display includes targets").map(drop)?;
    ensure(defaults.level(), "default display includes levels").map(drop)?;
    ensure(!defaults.thread_id(), "default display omits thread IDs").map(drop)?;
    ensure(!defaults.thread_name(), "default display omits thread names").map(drop)?;
    ensure(!defaults.filename(), "default display omits source files").map(drop)?;
    ensure(!defaults.line_number(), "default display omits source line numbers").map(drop)?;
    ...
    ensure(!customized.timestamp(), "timestamp flag can be disabled").map(drop)?;
    ensure(!customized.target(), "target flag can be disabled").map(drop)?;
    ensure(!customized.level(), "level flag can be disabled").map(drop)?;
    ensure(customized.thread_id(), "thread ID flag can be enabled").map(drop)?;
    ensure(customized.thread_name(), "thread name flag can be enabled").map(drop)?;
    ensure(customized.filename(), "source file flag can be enabled").map(drop)?;
    ensure(customized.line_number(), "line number flag can be enabled")
      .map(drop)
      .map_err(TestError::from)
  }

  #[test]
  fn format_builders_preserve_kind_and_toggle_display_options() -> Result<(), TestError> {
    ...
    ensure(!configured.display.timestamp(), "without_time disables timestamps").map(drop)?;
    ensure(!configured.display.target(), "with_target disables targets").map(drop)?;
    ensure(!configured.display.level(), "with_level disables levels").map(drop)?;
    ensure(configured.display.thread_id(), "with_thread_ids enables thread IDs").map(drop)?;
    ensure(configured.display.thread_name(), "with_thread_names enables thread names").map(drop)?;
    ensure(configured.display.filename(), "with_source_location enables file display").map(drop)?;
    ensure(configured.display.line_number(), "with_source_location enables line-number display").map(drop)?;

    let hidden_location = configured.with_source_location(false);
    ensure(!hidden_location.display.filename(), "with_source_location disables file display").map(drop)?;
    ensure(
      !hidden_location.display.line_number(),
      "with_source_location disables line-number display",
    )
    .map(drop)
    .map_err(TestError::from)
  }
  ...
  #[cfg(feature = "json")]
  #[test]
  fn json_format_builder_toggles_structured_output_options() -> Result<(), TestError> {
    let configured = format()
      .json()
      .flatten_event(true)
    ...
    ensure(configured.kind.flattens_event(), "flatten_event enables root event fields").map(drop)?;
    ensure(
      !configured.kind.displays_current_span(),
      "with_current_span disables current-span output",
    )
    .map(drop)?;
    ensure(!configured.kind.displays_span_list(), "with_span_list disables span-list output")
      .map(drop)
      .map_err(TestError::from)
  }

  #[test]
  fn timestamp_formatting_falls_back_and_respects_disabled_timestamps() -> Result<(), TestError> {
    let enabled = Format {
      kind:    Full,
      timer:   FailingTime,
      ansi:    None,
    ...
    let mut enabled_output = String::new();
    let enabled_result = {
      let mut enabled_writer = Writer::new(&mut enabled_output);
      ensure_ok(
        enabled.format_timestamp(&mut enabled_writer),
        "failing timers fall back to unknown time",
      )?;
      Ok::<(), TestError>(())
    };
    enabled_result?;
    ensure_eq(
      String::from(enabled_output.as_str()),
      String::from("<unknown time> "),
      "failing timestamp writes fallback text",
    )
    .map(drop)?;
    ...
    let disabled = Format {
      ...
      timer:   FailingTime,
      ansi:    None,
      display: FormatDisplay::default().with_timestamp(false),
    };
    let mut disabled_output = String::new();
    let disabled_result = {
      let mut disabled_writer = Writer::new(&mut disabled_output);
      ensure_ok(
        disabled.format_timestamp(&mut disabled_writer),
        "disabled timestamps skip failing timers",
      )?;
      Ok::<(), TestError>(())
    };
    disabled_result?;
    ...
    ensure_eq(
      String::from(disabled_output.as_str()),
      String::new(),
      "disabled timestamps write no prefix",
    )
    .map(drop)
    .map_err(TestError::from)
  }
  ...
  #[test]
  fn field_function_formatter_forwards_each_recorded_field() -> Result<(), TestError> {
    let make_writer = MockMakeWriter::default();
    let field_formatter =
      debug_fn(|writer: &mut Writer<'_>, field: &Field, value: &dyn Debug| write!(writer, "{}={};", field.name(), DebugValue(value)));
    ...
    with_default(subscriber, || {
      tracing::info!(answer = 42_i64, enabled = true, "field fn event");
    });

    let output = make_writer.get_string();
    ensure_contains(
      output.clone(),
      String::from("message=field fn event;"),
      "field function receives message fields",
    )
    .map(drop)?;
    ensure_contains(output.clone(), String::from("answer=42;"), "field function receives numeric fields").map(drop)?;
    ensure_contains(output, String::from("enabled=true;"), "field function receives boolean fields")
      .map(drop)
      .map_err(TestError::from)
  }

  #[test]
  fn field_function_formatter_controls_rendered_values() -> Result<(), TestError> {
    let make_writer = MockMakeWriter::default();
    let field_formatter =
      debug_fn(|writer: &mut Writer<'_>, field: &Field, _value: &dyn Debug| write!(writer, "{}=<redacted>;", field.name()));
    let subscriber = Subscriber::builder()
      .fmt_fields(field_formatter)
      .with_writer(make_writer.clone())
      .without_time()
    ...
    with_default(subscriber, || {
      tracing::info!(secret = "token", "redacted event");
    });

    let output = make_writer.get_string();
    ensure_contains(
      output.clone(),
      String::from("message=<redacted>;"),
      "field function controls message rendering",
    )
    .map(drop)?;
    ensure_contains(
      output.clone(),
      String::from("secret=<redacted>;"),
      "field function controls field rendering",
    )
    .map(drop)?;
    ensure(!output.contains("token"), "field function can suppress raw field values")
      .map(drop)
      .map_err(TestError::from)
  }

  #[test]
  fn function_pointer_event_formatter_forwards_context_and_event() -> Result<(), TestError> {
    let make_writer = MockMakeWriter::default();
    let formatter: TestEventFormatter = |ctx, mut writer, event| {
      write!(writer, "fn {} ", event.metadata().level())?;
      ctx.format_fields(writer.by_ref(), event)?;
      writeln!(writer)
    };
    let subscriber = Subscriber::builder()
    ...
    with_default(subscriber, || {
      tracing::info!("pointer formatter");
    });

    let output = make_writer.get_string();
    ensure_eq(
      String::from(output.as_str()),
      String::from("fn INFO pointer formatter\n"),
      "function pointer event formatter receives event metadata and context fields",
    )
    .map(drop)
    .map_err(TestError::from)
  }
  ...
  #[test]
  fn default_fields_add_recorded_span_fields_to_existing_output() -> Result<(), TestError> {
    let make_writer = MockMakeWriter::default();
    let subscriber = Subscriber::builder()
      .with_writer(make_writer.clone())
      .without_time()
    ...
    with_default(subscriber, || {
      let span = tracing::info_span!("recorded", first = 1, second = Empty);
      let _recorded_span = span.record("second", 2);
      let _entered_span = span.enter();
      tracing::info!("inside recorded span");
    });

    let output = make_writer.get_string();
    ensure_contains(
      output,
      String::from("recorded{first=1 second=2}: inside recorded span"),
      "recorded span fields are appended to previously formatted fields",
    )
    .map(drop)
    .map_err(TestError::from)
  }
  ...
  #[test]
  fn default_fields_render_error_sources_and_raw_field_names() -> Result<(), TestError> {
    #[derive(Debug)]
    struct InnerError;
    ...
    with_default(subscriber, || {
      tracing::error!(error = error_ref, r#type = "config", "operation failed");
    });
    ...
    ensure_contains(
      ...
      "default fields render message text",
    )
    .map(drop)?;
    ensure_contains(
      output.clone(),
      String::from("error=outer failure"),
      "default fields render error display",
    )
    .map(drop)?;
    ensure_contains(
      output.clone(),
      String::from("error.sources=[inner failure]"),
      "default fields render error source chains",
    )
    .map(drop)?;
    ensure_contains(
      output,
      String::from("type=\"config\""),
      "default fields strip raw identifier prefixes",
    )
    .map(drop)
    .map_err(TestError::from)
  }
  ...
  #[test]
  fn thread_name_display_aligns_to_longest_observed_name() -> Result<(), TestError> {
    let mut short = String::new();
    ensure_ok(write!(&mut short, "{}", FmtThreadName::new("fmt")), "short thread name formats")?;
    let mut long = String::new();
    ensure_ok(
      write!(&mut long, "{}", FmtThreadName::new("fmt-worker")),
      "long thread name formats",
    )?;

    ensure(short.ends_with("fmt"), "short thread names keep their suffix").map(drop)?;
    ensure_eq(
      String::from(long.as_str()),
      String::from("fmt-worker"),
      "longest observed thread name is not padded",
    )
    .map(drop)
    .map_err(TestError::from)
  }
  ...
  #[cfg(feature = "ansi")]
  fn test_ansi<T>(is_ansi: bool, expected: &str, builder: SubscriberBuilder<DefaultFields, Format<T>>) -> Result<(), TestError>
  where
    Format<T, MockTime>: FormatEvent<crate::Registry, DefaultFields>,
    T: Send + Sync + 'static,
  {
    let make_writer = MockMakeWriter::default();
    let subscriber = builder.with_writer(make_writer.clone()).with_ansi(is_ansi).with_timer(MockTime);
    assert_info_hello(subscriber, &make_writer, expected)
  }

  #[cfg(not(feature = "ansi"))]
  fn test_without_ansi<T>(expected: &str, builder: SubscriberBuilder<DefaultFields, Format<T>>) -> Result<(), TestError>
  where
    Format<T, MockTime>: FormatEvent<crate::Registry, DefaultFields>,
    T: Send + Sync + 'static,
  {
  ...
  fn test_without_level<T>(expected: &str, builder: SubscriberBuilder<DefaultFields, Format<T>>) -> Result<(), TestError>
  where
    Format<T, MockTime>: FormatEvent<crate::Registry, DefaultFields>,
    T: Send + Sync + 'static,
  {
  ...
  #[test]
  fn with_line_number_and_file_name() -> Result<(), TestError> {
    let make_writer = MockMakeWriter::default();
    let subscriber = Subscriber::builder()
      .with_writer(make_writer.clone())
      .with_file(true)
    ...
    ensure(expected.is_match(&result), "line and file output matches expected shape")
      .map(drop)
      .map_err(TestError::from)
  }

  #[test]
  fn with_line_number() -> Result<(), TestError> {
    let make_writer = MockMakeWriter::default();
    let subscriber = Subscriber::builder()
      .with_writer(make_writer.clone())
      .with_line_number(true)
  ...
  #[test]
  fn with_filename() -> Result<(), TestError> {
    let make_writer = MockMakeWriter::default();
    let subscriber = Subscriber::builder()
      .with_writer(make_writer.clone())
      .with_file(true)
  ...
  #[test]
  fn with_thread_ids() -> Result<(), TestError> {
    let make_writer = MockMakeWriter::default();
    let subscriber = Subscriber::builder()
      .with_writer(make_writer.clone())
      .with_thread_ids(true)
      .with_ansi(false)
      .with_timer(MockTime);
    let expected = "fake time  INFO ThreadId(NUMERIC) tracing_subscriber::fmt::format::test: hello\n";

    assert_info_hello_ignore_numeric(subscriber, &make_writer, expected)
  }

  /// Renders the default pretty formatter and checks the `INFO`/source-location layout.
  #[cfg(feature = "ansi")]
  fn ensure_pretty_default_output() -> Result<(), TestError> {
    let make_writer = MockMakeWriter::default();
    let subscriber = Subscriber::builder()
      .pretty()
      .with_writer(make_writer.clone())
  ...
  #[test]
  #[cfg(feature = "ansi")]
  fn pretty_default() -> Result<(), TestError> {
    ensure_pretty_default_output()
  }

  fn assert_info_hello(subscriber: impl Into<Dispatch>, buf: &MockMakeWriter, expected: &str) -> Result<(), TestError> {
    let _default = set_default(&subscriber.into());
    tracing::info!("hello");
    ...
    ensure_eq(
      ...
      "formatted event output matches",
    )
    .map(drop)
    .map_err(TestError::from)
  }

  // When numeric characters are used they often form a non-deterministic value as they usually
  // represent things like a thread id or line number. This assert method should be used when
  // non-deterministic numeric characters are present.
  fn assert_info_hello_ignore_numeric(subscriber: impl Into<Dispatch>, buf: &MockMakeWriter, expected: &str) -> Result<(), TestError> {
    let _default = set_default(&subscriber.into());
    tracing::info!("hello");
    ...
    ensure_eq(
      String::from(result_cleaned.as_ref()),
      String::from(expected),
  ...
  fn test_overridden_parents<T>(expected: &str, builder: SubscriberBuilder<DefaultFields, Format<T>>) -> Result<(), TestError>
  where
    Format<T, MockTime>: FormatEvent<crate::Registry, DefaultFields>,
    T: Send + Sync + 'static,
  {
    ...
    T: Send + Sync + 'static,
  {
    ...
    with_default(subscriber, || -> Result<(), TestError> {
      ...
      ensure_eq(
        String::from(scoped_output.as_str()),
        String::from(expected1),
      ...
      ensure_eq(
        String::from(overridden_output.as_str()),
        String::from(expected2),
  ...
  mod default {
    ...
    #[cfg(feature = "ansi")]
    #[test]
    fn with_ansi_true() -> Result<(), TestError> {
      let expected = "\u{1b}[2mfake time\u{1b}[0m \u{1b}[32m INFO\u{1b}[0m \
                      \u{1b}[2mtracing_subscriber::fmt::format::test\u{1b}[0m\u{1b}[2m:\u{1b}[0m hello\n";
      test_ansi(true, expected, Subscriber::builder())
    }

    #[cfg(feature = "ansi")]
    #[test]
    fn with_ansi_false() -> Result<(), TestError> {
    ...
    #[test]
    fn overridden_parents_in_scope() -> Result<(), TestError> {
  ...
  #[test]
  fn format_nanos() -> Result<(), TestError> {
  ...
  #[test]
  fn fmt_span_combinations() -> Result<(), TestError> {
    let none_flags = FmtSpan::NONE;
    ensure(!none_flags.contains(FmtSpan::NEW), "FmtSpan::NONE excludes new events").map(drop)?;
    ensure(!none_flags.contains(FmtSpan::ENTER), "FmtSpan::NONE excludes enter events").map(drop)?;
    ensure(!none_flags.contains(FmtSpan::EXIT), "FmtSpan::NONE excludes exit events").map(drop)?;
    ensure(!none_flags.contains(FmtSpan::CLOSE), "FmtSpan::NONE excludes close events").map(drop)?;

    let active_flags = FmtSpan::ACTIVE;
    ensure(!active_flags.contains(FmtSpan::NEW), "FmtSpan::ACTIVE excludes new events").map(drop)?;
    ensure(active_flags.contains(FmtSpan::ENTER), "FmtSpan::ACTIVE includes enter events").map(drop)?;
    ensure(active_flags.contains(FmtSpan::EXIT), "FmtSpan::ACTIVE includes exit events").map(drop)?;
    ensure(!active_flags.contains(FmtSpan::CLOSE), "FmtSpan::ACTIVE excludes close events").map(drop)?;

    let full_flags = FmtSpan::FULL;
    ensure(full_flags.contains(FmtSpan::NEW), "FmtSpan::FULL includes new events").map(drop)?;
    ensure(full_flags.contains(FmtSpan::ENTER), "FmtSpan::FULL includes enter events").map(drop)?;
    ensure(full_flags.contains(FmtSpan::EXIT), "FmtSpan::FULL includes exit events").map(drop)?;
    ensure(full_flags.contains(FmtSpan::CLOSE), "FmtSpan::FULL includes close events").map(drop)?;

    let new_close_flags = FmtSpan::NEW | FmtSpan::CLOSE;
    ensure(new_close_flags.contains(FmtSpan::NEW), "new-close combination includes new events").map(drop)?;
    ensure(
  ...
  fn current_path() -> Result<String, TestError> {
    let owned_path = Path::new("tracing-subscriber").join("src").join("fmt").join("format.rs");
    let module_path = ensure_some(owned_path.to_str(), "test module path does not contain invalid unicode").map_err(|failure| {
