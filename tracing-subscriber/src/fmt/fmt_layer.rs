...
use crate::fmt::FormatFields;
use crate::fmt::MakeWriter;
use crate::fmt::TestWriter;
...
#[derive(Debug)]
pub struct Layer<S, N = format::DefaultFields, E = format::Format<format::Full>, W = fn() -> io::Stdout> {
...
impl LayerFlags {
  ...
  const fn with_flag(self, bit: u8, enabled: bool) -> Self {
    let bits = if enabled { self.bits | bit } else { self.bits & !bit };
    Self {
      bits,
    }
  }
...
// This needs to be a seperate impl block because they place different bounds on the type
// parameters.
  ...
  W: for<'writer> MakeWriter<'writer> + 'static,
{
  ...
  /// };
  ///
  /// let layer = fmt::layer().event_format(format().compact());
  ...
  ///
  /// ```rust
  /// let layer = tracing_subscriber::fmt::layer().map_event_format(|e| e.compact());
  /// # // this is necessary for type inference.
  /// # use tracing_subscriber::Layer as _;
  /// # let _ = layer.with_subscriber(tracing_subscriber::registry::Registry::default());
  /// ```
...
impl<S, N, E, W> Layer<S, N, E, W> {
  ...
  /// let layer = fmt::layer().map_writer(move |w| stderr.or_else(w));
  /// # // this is necessary for type inference.
  /// # use tracing_subscriber::Layer as _;
  /// # let _ = layer.with_subscriber(tracing_subscriber::registry::Registry::default());
  /// ```
  ...
  /// let layer = tracing_subscriber::fmt::layer().map_fmt_fields(|f| f.debug_alt());
  /// # // this is necessary for type inference.
  /// # use tracing_subscriber::Layer as _;
  /// # let _ = layer.with_subscriber(tracing_subscriber::registry::Registry::default());
  /// ```
  ...
  N: for<'writer> FormatFields<'writer> + 'static,
{
  ...
  ///
  /// - `FmtSpan::NONE`: No events will be synthesized when spans are created, entered, exited, or
  ///   closed. Data from spans will still be included as the context for formatted events. This is
  ///   the default.
  ...
  /// - `FmtSpan::CLOSE`: An event will be synthesized when a span closes. If [timestamps are
  ///   enabled][time] for this formatter, the generated event will contain fields with the span's
  ///   _busy time_ (the total time for which it was entered) and _idle time_ (the total time that
  ///   the span existed but was not entered).
  /// - `FmtSpan::ACTIVE`: Events will be synthesized when spans are entered or exited.
  /// - `FmtSpan::FULL`: Events will be synthesized whenever a span is created, entered, exited, or
  ///   closed. If timestamps are enabled, the close event will contain the span's busy and idle
  ...
  ///
  /// let subscriber = fmt().with_span_events(FmtSpan::NEW | FmtSpan::CLOSE).finish();
  /// ```
  ///
  ...
  /// Sets the layer being built to use an [excessively pretty, human-readable
  ...
  /// # Options
  ///
  /// - [`Layer::flatten_event`] can be used to enable flattening event fields into the root object.
  ///
...
fn log_internal_write_error(enabled: bool, result: io::Result<()>, context: &str) {
  ...
  if let Err(error) = result {
    write_internal_error(format_args!("[tracing-subscriber] {context} Error: {error}\n"));
  }
}
  ...
  W: for<'writer> MakeWriter<'writer> + 'static,
{
  fn on_new_span(&self, attrs: &Attributes<'_>, id: Id, ctx: Context<'_, S>) -> SubscriberResult {
    ...
    if extensions.get_mut::<FormattedFields<N>>().is_none() {
      ...
      if self.fmt_fields.format_fields(fields.as_writer(), attrs).is_ok() {
    ...
    if self.fmt_span.fmt_timing && self.fmt_span.trace_close() && extensions.get_mut::<Timings>().is_none() {
  ...
  fn on_record(&self, id: Id, values: &Record<'_>, ctx: Context<'_, S>) -> SubscriberResult {
    ...
    fields.was_ansi = self.flags.is_ansi();
    fields.was_ansi_sanitized = self.flags.ansi_sanitization();
    if self.fmt_fields.format_fields(fields.as_writer(), values).is_ok() {
      let mut extensions = span.extensions_mut();
      let _previous = extensions.insert(fields);
    }
  ...
  fn on_close(&self, id: Id, ctx: Context<'_, S>) -> SubscriberResult {
    ...
    if let Some(timing) = extensions.get::<Timings>() {
      ...
      with_event_from_span!(id, span, "message" = "close", "time.busy" = t_busy, "time.idle" = t_idle, |event| {
        drop(extensions);
        drop(span);
        self.on_event(&event, ctx)?;
      });
  ...
  fn on_event(&self, event: &Event<'_>, ctx: Context<'_, S>) -> SubscriberResult {
    ...
    BUF.with(|thread_local_cell| {
      let mut event_buffer = thread_local_cell
        .try_borrow_mut()
        .map_or_else(|_| EventBuffer::Fallback(String::new()), EventBuffer::ThreadLocal);
      let buf = event_buffer.as_mut_string();

      let format_context = self.make_ctx(ctx, event);
      match self.fmt_event.format_event(
        &format_context,
        ...
        Err(_) if self.flags.log_internal_errors() => {
          let err_msg = format!("Unable to format the following event. Name: {}\n", event.metadata().name());
          let mut writer = self.make_writer.make_writer_for(event.metadata());
...
mod test {
  ...
  use crate::fmt::format::test::MockTime;
  use crate::fmt::format;
  use crate::fmt::layer::Layer as _;
  ...
  enum TestError {
    ...
    /// Preserves the complete native failure and its inputs.
    #[error(transparent)]
    ResultReloadError(#[from] strict_test_support::ResultFailure<reload::Error>),
    /// Preserves the complete native failure and its inputs.
    #[error(transparent)]
    ResultRegexError(#[from] strict_test_support::ResultFailure<regex::Error>),
  ...
  #[test]
  fn fmt_layer_downcasts() -> Result<(), TestError> {
    let default_format = Format::default();
  ...
  #[test]
  fn fmt_layer_downcasts_to_parts() -> Result<(), TestError> {
    let default_format = Format::default();
    let fmt_layer = Layer::default().event_format(default_format);
    let subscriber = fmt_layer.with_subscriber(Registry::default());
    let dispatch = Dispatch::new(subscriber);
    ...
    .map(drop)?;
    ensure(dispatch.downcast_ref::<Format>().is_some(), "dispatch downcasts to default format")
      .map(drop)
      .map_err(TestError::from)
  }
  ...
  fn sanitize_timings(input: &str) -> Result<String, TestError> {
  ...
  #[test]
  fn format_error_print_to_stderr() -> Result<(), TestError> {
    struct AlwaysError;
    ...
    ensure(actual.as_str().starts_with(expected), "format errors are written to stderr")
      .map(drop)
      .map_err(TestError::from)
  }

  #[test]
  fn format_error_ignore_if_log_internal_errors_is_false() -> Result<(), TestError> {
    struct AlwaysError;
  ...
  #[test]
  fn synthesize_span_none() -> Result<(), TestError> {
    let make_writer = MockMakeWriter::default();
    let subscriber = fmt::Subscriber::builder()
            .with_writer(make_writer.clone())
            .with_level(false)
  ...
  /// Installs an fmt subscriber emitting `span_events` and compares synthesized output to
  /// `expected`.
  fn ensure_synthesized_span_events(span_events: FmtSpan, expected: &'static str, message: &'static str) -> Result<(), TestError> {
    let make_writer = MockMakeWriter::default();
    let subscriber = fmt::Subscriber::builder()
      .with_writer(make_writer.clone())
      .with_level(false)
    ...
    let actual = sanitize_timings(&make_writer.get_string())?;
    ensure_eq(String::from(actual.as_str()), String::from(expected), message)
      .map(drop)
      .map_err(TestError::from)
  }

  #[test]
  fn synthesize_span_active() -> Result<(), TestError> {
  ...
  #[test]
  fn synthesize_span_close() -> Result<(), TestError> {
    let expected = "fake time span1{x=42}: tracing_subscriber::fmt::fmt_layer::test: close timing timing\n";
    ensure_synthesized_span_events(FmtSpan::CLOSE, expected, "close span events include timings")
  }

  #[test]
  fn synthesize_span_close_no_timing() -> Result<(), TestError> {
    let make_writer = MockMakeWriter::default();
    let subscriber = fmt::Subscriber::builder()
      .with_writer(make_writer.clone())
      .with_level(false)
  ...
  #[test]
  fn synthesize_span_full() -> Result<(), TestError> {
    let expected = "fake time span1{x=42}: tracing_subscriber::fmt::fmt_layer::test: new\nfake time span1{x=42}: \
                    tracing_subscriber::fmt::fmt_layer::test: enter\nfake time span1{x=42}: tracing_subscriber::fmt::fmt_layer::test: \
  ...
  #[test]
  fn make_writer_based_on_meta() -> Result<(), TestError> {
    struct MakeByTarget {
    ...
    ensure_eq(
      String::from(writer1_output.as_str()),
      String::from(expected_writer1),
    ...
    ensure_eq(
      String::from(writer2_output.as_str()),
      String::from(expected_writer2),
  ...
  #[test]
  fn recorded_span_fields_are_appended_to_formatted_context() -> Result<(), TestError> {
    let make_writer = MockMakeWriter::default();
    ...
    let expected = "fake time span1{first=1 second=2}: tracing_subscriber::fmt::fmt_layer::test: hello\n";
    ensure_eq(
      String::from(make_writer.get_string().as_str()),
      String::from(expected),
  ...
  #[test]
  fn layer_writer_mutation_and_sanitization_settings_affect_output() -> Result<(), TestError> {
    ...
    *layer.writer_mut() = replacement_writer.clone();
    ...
    ensure_eq(
      String::from(original_writer.get_string().as_str()),
      String::new(),
    ...
    ensure_contains(
      output,
      String::from("\u{1b}[31mred\u{1b}[0m"),
  ...
  #[test]
  fn mapped_layer_components_transform_writer_fields_and_events() -> Result<(), TestError> {
    let original_writer = MockMakeWriter::default();
    let replacement_writer = MockMakeWriter::default();
    let field_formatter = format::debug_fn(|writer: &mut format::Writer<'_>, field: &field::Field, value: &dyn StdDebug| {
    ...
    let layer = Layer::<Registry>::new()
      ...
      .map_event_format(|formatter| formatter.without_time().with_level(false).with_target(false).compact())
  ...
  #[cfg(feature = "ansi")]
  #[test]
  fn layer_no_color() -> Result<(), TestError> {
    let cases = [
    ...
    for (var, expected_ansi) in cases {
      ensure_eq(default_ansi_enabled(var), expected_ansi, "default ansi state follows NO_COLOR").map(drop)?;

      let no_color_layer: Layer<()> = Layer::default_with_no_color(var);
      ensure_eq(no_color_layer.flags.is_ansi(), expected_ansi, "layer ansi state follows NO_COLOR").map(drop)?;

      // with_ansi should override any `NO_COLOR` value
      let overridden_layer: Layer<()> = Layer::default_with_no_color(var).with_ansi(true);
      ensure(overridden_layer.flags.is_ansi(), "with_ansi overrides NO_COLOR").map(drop)?;
      ...
      ensure(mutable_layer.flags.is_ansi(), "set_ansi overrides NO_COLOR").map(drop)?;
    }
    Ok(())
  }
  ...
  #[test]
  fn modify_span_events() -> Result<(), TestError> {
    let make_writer = MockMakeWriter::default();
    ...
    with_default(reload, || -> Result<(), TestError> {
      {
        let span1 = tracing::info_span!("span1", x = 42);
        let _entered = span1.enter();
      }
    ...
    let actual = sanitize_timings(&make_writer.get_string())?;
    let expected = "fake time span1{x=42}: tracing_subscriber::fmt::fmt_layer::test: enter\nfake time span1{x=42}: \
                    tracing_subscriber::fmt::fmt_layer::test: exit\nfake time span3{x=42}: tracing_subscriber::fmt::fmt_layer::test: \
                    exit\n";
    ensure_eq(
      String::from(actual.as_str()),
      String::from(expected),
