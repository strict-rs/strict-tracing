...
impl FormatTime for ChronoUtc {
  fn format_time(&self, writer: &mut Writer<'_>) -> FmtResult {
    ...
    match *self.format.as_ref() {
      ...
      ChronoFmtType::Custom(ref format_string) => writer.write_str(&format!("{}", timestamp.format(format_string))),
    }
  }
}
...
mod tests {
  ...
  fn ensure_default_rfc3339<T: FormatTime>(timer: &T, write_message: &'static str, rfc_message: &'static str) -> Result<(), TestError> {
  ...
  fn ensure_custom_format<T: FormatTime>(timer: &T, write_message: &'static str, format_message: &'static str) -> Result<(), TestError> {
    let mut buf = String::new();
    let mut dst: Writer<'_> = Writer::new(&mut buf);
  ...
  #[test]
  fn test_chrono_format_time_utc_default() -> Result<(), TestError> {
  ...
  #[test]
  fn test_chrono_format_time_utc_custom() -> Result<(), TestError> {
    let fmt = ChronoUtc {
  ...
  #[test]
  fn test_chrono_format_time_local_default() -> Result<(), TestError> {
    ensure_default_rfc3339(
      &ChronoLocal::default(),
  ...
  #[test]
  fn test_chrono_format_time_local_custom() -> Result<(), TestError> {
    let fmt = ChronoLocal {
      format: Arc::new(ChronoFmtType::Custom("%a %b %e %T %Y".to_owned())),
    };
    ensure_custom_format(
