...
/// let subscriber = tracing_subscriber::fmt().with_writer(Arc::clone(&log_file)).finish();
/// # drop(subscriber);
/// # Ok(())
/// # }
/// ```
...
pub trait MakeWriter<'a> {
  ...
  /// # Implementer notes
  ///
  /// [`fmt::Layer`] or [`fmt::Subscriber`] will call this method each time an event is recorded.
  /// Ensure any state that must be saved across writes is not lost when the [`Writer`] instance
  /// is dropped. If creating a [`io::Write`] instance is expensive, be sure to cache it when
...
pub trait MakeWriterExt<'a>: MakeWriter<'a> {
  ...
  ///
  /// # fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
  /// let mk_writer = std::io::stderr.with_max_level(Level::WARN).or_else(std::io::stdout);
  ///
  /// tracing_subscriber::fmt().with_writer(mk_writer).try_init()?;
  /// # Ok(()) }
  /// ```
  ...
  B: MakeWriter<'a>,
{
  ...
  #[inline]
  fn make_writer_for(&'a self, meta: &Metadata<'_>) -> Self::Writer {
    Tee::new(self.first.make_writer_for(meta), self.second.make_writer_for(meta))
  }
}
...
impl Write for WriteAdaptor<'_> {
  fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
    ...
    self.fmt_write.write_str(text).map_err(io_error_from_fmt)?;
...
mod test {
  ...
  enum TestError {
    ...
    /// Preserves the complete native failure and its inputs.
    #[error(transparent)]
    OptionLevelString(#[from] strict_test_support::OptionFailure<(Level, String)>),
  }
  ...
  /// Installs a time-free fmt subscriber writing through `make_writer`, returning the default
  /// guard.
  ...
  fn test_writer<T>(make_writer: T, msg: &str, buf: &Mutex<Vec<u8>>) -> Result<(), TestError>
  where
    T: for<'writer> MakeWriter<'writer> + Send + Sync + 'static,
  {
    ...
    ensure(actual.contains(expected.as_str()), "custom writer output contains expected line")
      .map(drop)
      .map_err(TestError::from)
  }

  fn has_lines(buf: &Mutex<Vec<u8>>, msgs: &[(Level, &str)]) -> Result<(), TestError> {
    let actual = String::from_utf8_lossy(&buf.lock()).into_owned();
    let mut expected_lines = msgs.iter();
    for actual_line in actual.lines() {
      let line = actual_line.trim();
      let &(level, msg) = ensure_some(expected_lines.next(), "writer emitted no more lines than expected").map_err(|failure| {
        strict_test_support::OptionFailure {
          context: failure.context,
          option:  failure.option.map(|&(level, text)| (level, String::from(text))),
        }
      })?;
      let expected = format!("{} {}: {}", level, module_path!(), msg);
      ensure(line == expected.as_str(), "writer emitted expected line").map(drop)?;
    }
    ensure(expected_lines.next().is_none(), "writer emitted all expected lines")
      .map(drop)
      .map_err(TestError::from)
  }

  #[test]
  fn custom_writer_closure() -> Result<(), TestError> {
    let buf = Arc::new(Mutex::new(Vec::new()));
  ...
  #[test]
  fn custom_writer_struct() -> Result<(), TestError> {
    let buf = Arc::new(Mutex::new(Vec::new()));
  ...
  #[test]
  fn combinators_level_filters() -> Result<(), TestError> {
  ...
  #[test]
  fn combinators_or_else() -> Result<(), TestError> {
  ...
  #[test]
  fn combinators_or_else_chain() -> Result<(), TestError> {
    let (info_buf, info) = writer_buffer();
    let (debug_buf, debug) = writer_buffer();
    let (warn_buf, warn) = writer_buffer();
    let (err_buf, err) = writer_buffer();
    ...
    let make_writer = err.with_max_level(Level::ERROR).or_else(
      warn
        ...
        .or_else(info.with_max_level(Level::INFO).or_else(debug.with_max_level(Level::DEBUG))),
    );
  ...
  #[test]
  fn combinators_and() -> Result<(), TestError> {
  ...
  #[test]
  fn optional_writer_forwards_enabled_writes_and_discards_disabled_writes() -> Result<(), TestError> {
    ...
    match some {
      EitherWriter::First(bytes) => ensure(bytes == b"enabled".to_vec(), "enabled optional writer stores bytes")
        .map(drop)
        .map_err(TestError::from),
    ...
    .map(drop)?;
    ensure_ok(none.flush(), "disabled optional writer flushes").map_err(TestError::from)
  }

  #[test]
  fn option_converts_into_optional_writer_polarities() -> Result<(), TestError> {
    let some: OptionalWriter<Vec<u8>> = Some(b"present".to_vec()).into();
    let none: OptionalWriter<Vec<u8>> = None.into();

    match some {
      EitherWriter::First(bytes) => ensure(bytes == b"present".to_vec(), "Some converts to enabled writer")
        .map(drop)
        .map_err(TestError::from),
  ...
  #[test]
  fn level_combinators_enable_only_matching_metadata() -> Result<(), TestError> {
  ...
  #[test]
  fn filter_combinator_enables_only_matching_metadata() -> Result<(), TestError> {
    ...
    let filtered = filter_writer.with_filter(|meta| meta.target() == "writer_target");
      ...
      matches!(filtered.make_writer_for(WRITER_CALLSITE.metadata()), OptionalWriter::First(_)),
  ...
  #[test]
  fn tee_and_or_else_forward_write_methods_to_expected_branches() -> Result<(), TestError> {
    static INFO_META: Metadata<'static> = tracing_core::metadata! {
        name: "writer_info",
        target: "writer_target",
        level: Level::INFO,
    ...
    ensure(tee.first == b"tee-all-fmt".to_vec(), "tee writes first branch").map(drop)?;
    ensure(tee.second == b"tee-all-fmt".to_vec(), "tee writes second branch").map(drop)?;
    ...
    ensure_ok(primary_writer.write_all(b"primary"), "or-else writes primary branch")?;
    let mut fallback_writer = or_else.make_writer_for(&TRACE_META);
    ensure_ok(fallback_writer.write_all(b"fallback"), "or-else writes fallback branch")?;
  ...
  #[test]
  fn boxed_writer_forwards_type_erased_writes() -> Result<(), TestError> {
    static INFO_META: Metadata<'static> = tracing_core::metadata! {
        name: "writer_info",
        target: "writer_target",
        level: Level::INFO,
    ...
    ensure_ok(writer_for.write_all(b"for"), "boxed writer forwards metadata-specific write_all")?;
    ensure_eq(
  ...
  #[test]
  fn tee_make_writer_constructs_default_and_metadata_specific_writer_pairs() -> Result<(), TestError> {
    let (first_buf, first) = writer_buffer();
    let (second_buf, second) = writer_buffer();
    let tee = first.and(second);

    let mut default_writer = tee.make_writer();
    ensure_ok(default_writer.write_all(b"default"), "tee default writer forwards write_all")?;
    let mut metadata_writer = tee.make_writer_for(WRITER_CALLSITE.metadata());
    ensure_ok(metadata_writer.write_all(b"-metadata"), "tee metadata writer forwards write_all")?;
  ...
  #[test]
  fn or_else_default_writer_uses_fallback_when_primary_is_unclassified() -> Result<(), TestError> {
    let (primary_buf, primary) = writer_buffer();
    let (fallback_buf, fallback) = writer_buffer();
    let or_else = primary.with_max_level(Level::INFO).or_else(fallback);

    let mut writer = or_else.make_writer();
    ensure_ok(writer.write_all(b"fallback"), "or-else default writer uses fallback branch")?;

    ensure(primary_buf.lock().is_empty(), "disabled primary receives no unclassified bytes").map(drop)?;
    ensure_eq(
      String::from_utf8_lossy(&fallback_buf.lock()).into_owned(),
      String::from("fallback"),
  ...
  #[test]
  fn either_writer_forwards_vectored_all_fmt_and_flush_to_both_variants() -> Result<(), TestError> {
    ...
    match first {
      EitherWriter::First(bytes) => ensure(bytes == b"ab-all-fmt".to_vec(), "first either writer stores branch A bytes")
        .map(drop)
        .map_err(TestError::from),
    ...
    match second {
      EitherWriter::First(_) => ensure(false, "second either writer remains branch B")
        .map(drop)
        .map_err(TestError::from),
      EitherWriter::Second(bytes) => ensure(bytes == b"cd-all-fmt".to_vec(), "second either writer stores branch B bytes")
        .map(drop)
        .map_err(TestError::from),
    }
  }
  ...
  #[test]
  fn arc_make_writer_borrows_shared_writer_without_losing_state() -> Result<(), TestError> {
    #[derive(Default)]
    struct SharedWrite {
      bytes: Mutex<Vec<u8>>,
    }
  ...
  #[cfg(any(feature = "json", feature = "time"))]
  #[test]
  fn write_adaptor_forwards_utf8_and_reports_formatter_errors() -> Result<(), TestError> {
    struct FailingFmt;
    ...
    ensure_eq((output).clone(), String::from("utf8"), "write adaptor appends text").map(drop)?;

    let mut invalid_adaptor = WriteAdaptor::new(&mut output);
    ensure(invalid_adaptor.write(&[0xff]).is_err(), "write adaptor rejects invalid UTF-8 bytes").map(drop)?;
