...
- `fmt/` — `fmt::Layer<S, N, E, W>` (`fmt/fmt_layer.rs`) is the formatting layer, generic over field formatter `N: FormatFields` (`DefaultFields`), event formatter `E: FormatEvent` (`Format<Full>`; `Compact`/`Pretty`/`Json` in `fmt/format/`), and writer `W: MakeWriter` (`fmt/writer.rs`, default stdout). `fmt::Subscriber`/`FmtSubscriber` (`fmt/mod.rs`) bundles that layer over a `Registry` with an `EnvFilter`, built via `SubscriberBuilder` (`fmt()` / `.try_init()?`). Timestamps come from `FormatTime` impls in `fmt/time/` (`SystemTime`, `Uptime`, plus `time`/`chrono` crate formatters).
- `reload.rs` — `reload::Layer`/`Handle` wrap any layer to swap it at runtime (commonly to change a filter). `util.rs` adds `SubscriberInitExt` (`.try_init()`/`.set_default()`); under the `tracing-log` feature, `set_default()` initializes `LogTracer`. `field/` provides the `MakeVisitor`/`VisitFmt` field-visitor machinery used by formatters.
