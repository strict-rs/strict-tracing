...
fn none_max_level_hint() -> Result<(), TestFailure> {
  ...
  let filtered_fn_layer =
    raw_filter_fn_layer.with_filter(filter::dynamic_filter_fn(move |meta, _| meta.level() <= &max_level).with_max_level_hint(max_level));
  let filtered_fn_hint = ensure_ok(filtered_fn_layer.max_level_hint(), "filter function max level hint returns")?;
  ensure(
...
fn some_max_level_hint() -> Result<(), TestFailure> {
  ...
  let filtered_some_layer = raw_some_layer.with_filter(Some(
    filter::dynamic_filter_fn(move |meta, _| meta.level() <= &Level::DEBUG).with_max_level_hint(Level::DEBUG),
  ));
  let filtered_some_hint = ensure_ok(filtered_some_layer.max_level_hint(), "Some filter max level hint returns")?;
  ensure(
  ...
  let filtered_fn_layer = raw_filter_fn_layer
    .with_filter(filter::dynamic_filter_fn(move |meta, _| meta.level() <= &Level::INFO).with_max_level_hint(Level::INFO));
  let filtered_fn_hint = ensure_ok(filtered_fn_layer.max_level_hint(), "filter function max level hint returns")?;
  ensure(
    filtered_fn_hint == Some(LevelFilter::INFO),
    "filter function provides info max level hint",
