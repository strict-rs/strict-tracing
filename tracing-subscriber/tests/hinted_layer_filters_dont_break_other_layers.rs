...
mod tests {
  ...
  fn layer_filters() -> Result<(), TestFailure> {
    ...
    ensure_ok(filtered_handle.finished(), "mock expectations should finish")?;
    Ok(())
  }
  ...
  fn layered_layer_filters() -> Result<(), TestFailure> {
    ...
    let filtered = filtered1.with_filter(filter()).and_then(filtered2.with_filter(filter()));

    let subscriber = tracing_subscriber::registry().with(unfiltered).with(filtered);
    ensure(
      subscriber.max_level_hint().is_none(),
    ...
    ensure_ok(unfiltered1_handle.finished(), "mock expectations should finish")?;
    ensure_ok(unfiltered2_handle.finished(), "mock expectations should finish")?;
    ensure_ok(filtered1_handle.finished(), "mock expectations should finish")?;
    ensure_ok(filtered2_handle.finished(), "mock expectations should finish")?;
    Ok(())
  }
  ...
  fn filter<S>() -> DynFilterFn<S> {
    let filter: fn(&Metadata<'_>, &Context<'_, S>) -> bool = |metadata: &Metadata<'_>, _: &Context<'_, S>| metadata.level() <= &Level::INFO;
