...
mod tests {
  ...
  {
    fn enabled(&self, _metadata: &Metadata<'_>, _ctx: Context<'_, C>) -> SubscriberResult<bool> {
      Ok(self.enabled)
    }

    fn event_enabled(&self, _event: &Event<'_>, _ctx: Context<'_, C>) -> SubscriberResult<bool> {
