...
mod tests {
  ...
  impl Layer<CountingSubscriber> for CountingLayer {
    fn on_new_span(&self, _attrs: &span::Attributes<'_>, _id: Id, _ctx: Context<'_, CountingSubscriber>) -> SubscriberResult {
  ...
  fn span_entered_on_different_thread_from_subscriber() -> Result<(), TestFailure> {
    ...
    let span = with_default(Arc::clone(&subscriber), move || tracing::span!(Level::INFO, "span"));
