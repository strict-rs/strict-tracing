...
mod tests {
  ...
  fn subscriber_and_option_some_layer() -> Result<(), TestFailure> {
    ...
    ensure_hint(&subscriber, Some(LevelFilter::DEBUG), "Some layer overrides previous hint")
  }
  ...
  fn subscriber_and_option_none_layer() -> Result<(), TestFailure> {
    ...
    ensure_hint(&subscriber, Some(LevelFilter::ERROR), "None layer preserves previous hint")
  }
  ...
  fn none_outside_doesnt_override_max_level() -> Result<(), TestFailure> {
    ...
    let debug_inside = tracing_subscriber::registry()
      ...
      .with(None::<LevelFilter>);
    ensure_hint(&debug_inside, Some(LevelFilter::DEBUG), "outer None preserves inner debug hint")?;

    let filtered_none = tracing_subscriber::registry()
      .with(BasicLayer(None))
      .with(None::<LevelFilter>.with_filter(LevelFilter::DEBUG));
    ensure_hint(&filtered_none, None, "filtered outer None preserves inner None hint")?;
  ...
  fn none_inside_doesnt_override_max_level() -> Result<(), TestFailure> {
    ...
    let error_inside = tracing_subscriber::registry()
      .with(Some(LevelFilter::ERROR))
      .with(BasicLayer(None));
    ensure_hint(&error_inside, Some(LevelFilter::ERROR), "inner None preserves outer error hint")?;

    let debug_basic = tracing_subscriber::registry()
      .with(None::<LevelFilter>)
      .with(BasicLayer(Some(LevelFilter::DEBUG)));
    ensure_hint(&debug_basic, Some(LevelFilter::DEBUG), "inner debug hint wins over outer None")?;

    let filtered_none = tracing_subscriber::registry()
      .with(None::<LevelFilter>.with_filter(LevelFilter::DEBUG))
      .with(BasicLayer(None));
    ensure_hint(&filtered_none, None, "inner None wins over filtered outer None")?;
    ...
    let info_basic = tracing_subscriber::registry()
      .with(None::<LevelFilter>)
      .with(BasicLayer(Some(LevelFilter::INFO)));
    ensure_hint(&info_basic, Some(LevelFilter::INFO), "inner info hint wins over outer None")
  }
  ...
  fn reload_works_with_none() -> Result<(), TestFailure> {
    ...
    ensure_hint(&subscriber, Some(LevelFilter::OFF), "two empty reload layers start disabled")?;

    ensure_ok(handle1.reload(Some(BasicLayer(None))), "reload layer accepts a None hint layer")?;
    ensure_hint(&subscriber, None, "reloaded None hint passes through correctly")?;
