...
mod tests {
  ...
  mod my_module {
    ...
    pub(super) fn test_log_enabled() -> Result<(), TestFailure> {
      ensure(log::log_enabled!(log::Level::Info), "info is enabled inside `my_module`")?;
      ensure(!log::log_enabled!(log::Level::Debug), "debug is disabled inside `my_module`")?;
  ...
  fn log_is_enabled() -> Result<(), TestFailure> {
    ...
    ensure(!log::log_enabled!(log::Level::Info), "info is disabled outside `my_module`")?;
    ensure(!log::log_enabled!(log::Level::Warn), "warn is disabled outside `my_module`")?;
