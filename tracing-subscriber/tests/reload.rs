...
mod tests {
    ...
    use tracing_core::{
        Dispatch, Event, LevelFilter, Metadata, Subscriber,
    ...
    fn reload_handle() -> Result<(), TestFailure> {
        ...
        with_default(&subscriber, || -> Result<(), TestFailure> {
            ensure_eq(
                ...
                &0,
                "first reload layer starts with no calls",
            )?;
            ensure_eq(
            ...
            ensure_eq(
                &FILTER1_CALLS.load(Ordering::SeqCst),
                &1,
                "first reload layer sees the first event",
            )?;
            ensure_eq(
                &FILTER2_CALLS.load(Ordering::SeqCst),
                &0,
            ...
            ensure_ok(
                handle.reload(Filter::Two),
                "reload layer swaps to second filter",
            )?;
            ensure_eq(
                &LevelFilter::current(),
                &LevelFilter::DEBUG,
                "reloaded layer max level is current",
            )?;
    ...
    fn reload_filter() -> Result<(), TestFailure> {
        ...
        let dispatcher =
            Dispatch::new(tracing_subscriber::registry().with(NopLayer.with_filter(filter)));
        ...
        with_default(&dispatcher, || -> Result<(), TestFailure> {
            ...
            ensure_ok(
                handle.reload(Filter::Two),
                "reload filter swaps to second filter",
            )?;
            ensure_eq(
                &LevelFilter::current(),
                &LevelFilter::DEBUG,
                "reloaded filter max level is current",
            )?;
