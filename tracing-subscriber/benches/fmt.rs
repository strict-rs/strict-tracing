...
fn bench_thrpt(
    ...
    name: &'static str,
    mut register: impl FnMut(&mut Group<'_>, &usize),
) {
    ...
    register_benchmark(group.bench_with_input(
        ...
        |bencher, &count| {
            bencher.iter_custom(|iters| {
                ...
                let mut total = Duration::ZERO;
                for _ in 0..iters {
                    let parent = with_default(&dispatch, || {
...
fn register_shared_parent_worker(
...
fn register_multi_parent_worker(
    bench: &MultithreadedBench,
    parent: tracing::Span,
    event_count: usize,
) {
